...
@media (min-width: 1024px) {
  .container { max-width: 540px; padding: 24px; }
  .hotel-list { grid-template-columns: repeat(2, 1fr); display: grid; gap: 24px; }
  .custom-options { flex-direction: row; }
}
@media (min-width: 768px) {
  .hotel-list { grid-template-columns: repeat(2, 1fr); display: grid; gap: 16px; }
...
document.body.innerHTML = `
  <div class="container">
    ...
    <div id="step2" class="step">
      ...
      <label class="upload">
        ...
        <div class="circle-upload" id="profileCircle"><svg width="24" height="24" fill="var(--primary-color)"><path d="M12 5v14M5 12h14"/></svg><div class="spinner"></div></div>
    ...
    <div id="step6" class="step">
      ...
      <label class="upload">
        ...
        <div class="circle-upload" id="docFront"><svg width="24" height="24" fill="var(--primary-color)"><path d="M12 5v14M5 12h14"/></svg><div class="spinner"></div></div>
        <div class="upload-text">Frente do documento</div>
      </label>
      <label class="upload">
        <input type="file" id="docBackInput" accept="image/jpeg,image/png">
        <div class="circle-upload" id="docBack"><svg width="24" height="24" fill="var(--primary-color)"><path d="M12 5v14M5 12h14"/></svg><div class="spinner"></div></div>
        <div class="upload-text">Verso do documento</div>
      </label>
      <button class="btn" id="step6-btn" disabled>Prosseguir</button>
    </div>
  ...
  <div class="date-fullscreen" id="date-fullscreen">
    <div class="calendar-header">Selecione as datas da experiência</div>
    <div class="calendar-content" id="calendar-content">
      <button class="load-more-dates" id="load-more-dates">Carregar mais datas</button>
    </div>
  ...
  <div id="custom-alert-popup">
    <p id="alert-message"></p>
    <button class="btn secondary" onclick="closeCustomAlert()">Fechar</button>
  </div>
  <div id="block-popup">
    <p>Bloqueado por 24 horas devido a violações. Tempo restante: <span id="block-timer"></span></p>
  </div>
`;
// ===== Lógica JavaScript =====
...
let blockTimestamp = localStorage.getItem('blockTimestamp');
const popularCities = [
  { name: 'São Paulo, SP', label: 'Mais acessada' },
  { name: 'Rio de Janeiro, RJ', label: 'Melhores hotéis' },
  { name: 'Brasília, DF', label: 'Mais acessadas' }
];
function sanitizeInput(input) {
  return input.replace(/<script.*?>.*?<\/script>/gi, '').replace(/[<>&'"]/g, char => ({'<':'&lt;','>':'&gt;','&':'&amp;',"'":'&#39;,'"':'&quot;'}[char]));
}
function checkLocalStorageLimit() {
  const size = new Blob(Object.values(localStorage)).size / 1024 / 1024;
  if (size > 4.5) {
    localStorage.clear();
    showToast('Armazenamento local limpo para continuar a operação.');
  }
}
...
function checkProhibited(value) {
  const lowerValue = value.toLowerCase();
  if (prohibitedWords.some(word => lowerValue.includes(word))) {
    warningCount++;
    localStorage.setItem('warningCount', warningCount);
    if (warningCount >= 3) {
      const now = Date.now();
      localStorage.setItem('blockTimestamp', now + 24 * 60 * 60 * 1000);
      showBlockPopup();
      return true;
    } else {
...
function showCustomAlert(msg) {
  ...
  document.getElementById('custom-alert-popup').style.display = 'block';
}
...
function showBlockPopup() {
  ...
  updateBlockTimer();
  setInterval(updateBlockTimer, 60000);
  document.querySelector('.container').style.pointerEvents = 'none';
}
function updateBlockTimer() {
  const end = parseInt(localStorage.getItem('blockTimestamp'));
  ...
  if (remaining <= 0) {
    ...
    document.getElementById('block-timer').textContent = `${hours}h ${mins}m`;
  }
}
...
function previewImage(event, targetId) {
  ...
  if (file && validTypes.includes(file.type)) {
    ...
    reader.onload = (e) => {
      circle.innerHTML = `<img src="${e.target.result}" alt="Imagem" loading="lazy"> <div class="spinner"></div>`;
    ...
    showCustomAlert('Por favor, envie uma imagem no formato JPEG ou PNG.');
    spinner.style.display = 'none';
  }
}
function checkDocsUploaded() {
  ...
  document.getElementById('step6-btn').disabled = !(front && back);
}
...
function updatePopularCities() {
  ...
  popularCities.forEach(c => {
    ...
    option.innerHTML = `<svg viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg><div>${c.name}<small>${c.label}</small></div>`;
...
function updateCalendar(monthsToRender = 2) {
  ...
  for (let i = 0; i < monthsToRender; i++) {
    ...
    section.innerHTML = `<div class="month-title">${new Date(year, month).toLocaleString('pt-BR', { month: 'long', year: 'numeric' })}</div>`;
...
function selectDate(date) {
  ...
  document.getElementById('calendar-content').innerHTML = '<button class="load-more-dates" id="load-more-dates">Carregar mais datas</button>';
  updateCalendar();
...
function initAutocomplete() {
  ...
  input.addEventListener('focus', () => {
    if (!input.value) updatePopularCities();
  });
  input.addEventListener('input', () => {
    const val = removeAccents(input.value.toLowerCase());
    list.innerHTML = '';
    if (!val) {
      updatePopularCities();
      return;
    }
    cities.filter(c => removeAccents(c.toLowerCase()).includes(val)).forEach(c => {
      const option = document.createElement('div');
      option.className = 'city-option';
      option.innerHTML = `<svg viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>${c}`;
      option.addEventListener('click', () => {
        input.value = c;
        list.style.display = 'none';
        loadHotels(c);
    ...
    list.style.display = cities.filter(c => removeAccents(c.toLowerCase()).includes(val)).length ? 'block' : 'none';
  });
}
function loadHotels(city) {
  ...
  if (hotels[city]) {
    hotels[city].forEach((hotel, index) => {
      ...
      const isFavorited = userData.favorites.some(f => f.name === hotel.name);
      card.innerHTML = `
        <div class="hotel-img-container">
          <img src="${hotel.profileImg}" class="hotel-img" alt="${hotel.name}">
          <div class="hotel-selo">${hotel.selo}</div>
          <div class="hotel-favorite ${isFavorited ? 'filled' : ''}" onclick="toggleFavorite('${city}', ${index})">
...
function toggleFavorite(city, index) {
  const hotel = hotels[city][index];
  const favIndex = userData.favorites.findIndex(f => f.name === hotel.name);
  if (favIndex > -1) {
    userData.favorites.splice(favIndex, 1);
    showToast('Hotel removido dos favoritos.');
  } else {
    userData.favorites.push(hotel);
...
function showPaymentWaiting() {
  ...
  document.getElementById('step4').appendChild(btn);
}
function startVerification() {
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } })
    ...
    .catch(err => {
      showCustomAlert('Permissão para câmera negada. Por favor, permita o acesso para continuar.');
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
  if (blockTimestamp) showBlockPopup();
  updateProgress();
  showStep(currentStep);
  initAutocomplete();
  document.getElementById('dates').addEventListener('click', showCalendar);
  document.getElementById('load-more-dates').addEventListener('click', () => updateCalendar(1));
  ...
  document.getElementById('step6-btn').addEventListener('click', () => { if (validateStep(6)) showStep(7); });
  ...
  selectDoc.addEventListener('change', () => document.getElementById('step5-btn').disabled = !selectDoc.value);
