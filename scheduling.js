...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step4" class="step">
      ...
      <p>Seus documentos devem representar o proprietário do CPF indicado abaixo:</p>
    ...
    <div id="step6" class="step">
      ...
      <div class="upsell-group" onclick="toggleUpsell(this)">
        ...
        <div class="upsell-info">
          ...
          <div class="upsell-desc">Receba acesso a 1 diária integral no hotel parceiro após o término da experiência.</div>
        </div>
    ...
    <div id="step7" class="step">
      ...
      <p>Suas informações e escolhas foram enviadas com sucesso e estão em análise.</p>
      <p>Na próxima etapa, você realizará uma <strong>verificação facial</strong> para confirmar sua identidade de forma segura com nossa tecnologia, o Frame Authentic.</p>
  ...
  <div class="orientation-alert">
    <p>Olá! Por favor, utilize o seu dispositivo no modo vertical para melhor experiência nessa etapa do agendamento na Frame Agency.</p>
  </div>
  ...
  <div class="premium-overlay" id="premium-overlay">
    <div class="premium-content">
      ...
      <p id="decline-premium">Não desejo a oferta</p>
    </div>
  </div>
  <div class="hotel-fullscreen" id="hotel-fullscreen">
    <div class="calendar-header">
      <button class="btn secondary" id="close-hotels" style="font-size: 0.8rem; padding: 6px 12px;">Fechar</button>
      <div class="hotel-section-title" id="hotel-title">Hotéis parceiros em: </div>
    </div>
    <div id="hotel-grid" class="hotel-list"></div>
  </div>
`;
// ===== Lógica JavaScript =====
...
function checkProhibited(inputValue) {
  ...
  if (prohibitedWordsList.includes(texto)) {
    ...
    if (warningCount >= 4) {
      ...
      showWarningPopup(`Aviso ${warningCount}/3: Palavra inadequada detectada. Por favor, evite termos ou expressões inapropriadas no preenchimento do seu cadastro.`);
      return true;
    }
  }
  return false;
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (!cpfRegex.test(cpf) || !validateCPF(cpf)) {
      showError('cpf', 'Insira um CPF válido e tente novamente.');
      valid = false;
    }
    if (!rua) {
    ...
    if (!estado) {
      showError('estado', 'Selecione seu estado.');
      valid = false;
    }
    if (valid) {
...
function loadHotels(city) {
  const listContainer = document.getElementById('hotel-grid');
...
function handlePayment() {
  ...
  setTimeout(() => {
    ...
    confirmBtn.addEventListener('click', () => {
      ...
      showLoadingOverlay(['Verificando pagamento…', 'Enviando dados...'], () => showStep(7));
    });
  }, 1000);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('close-hotels').addEventListener('click', () => document.getElementById('hotel-fullscreen').classList.remove('show'));
