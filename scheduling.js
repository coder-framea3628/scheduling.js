...
.fr-photo-adding-label { font-family: "Poppins", sans-serif; font-size: 0.82rem; color: rgba(255,255,255,0.7); text-align: center; margin-top: 14px; opacity: 0; transition: opacity 0.5s ease; pointer-events: none; }
.fr-photo-adding-label.show { opacity: 1; }
/* === KYC SUCCESS SCREEN === */
...
.fr-msg-subq { margin-top: 16px; animation: frFadeIn 0.28s ease both; }
.fr-msg-subq-title { font-size: 0.88rem; font-weight: 600; color: var(--fr-charcoal); margin-bottom: 10px; }
.fr-msg-chips-sub { gap: 8px; }
.fr-msg-back-link { display: inline-block; margin-top: 14px; font-size: 0.8rem; color: var(--fr-bronze); cursor: pointer; background: none; border: none; font-family: 'Poppins', sans-serif; text-decoration: underline; padding: 0; }
.fr-chip-expandable.expanded { border-color: var(--fr-bronze); background: var(--fr-bronze-pale); color: var(--fr-bronze); font-weight: 600; }
  ...
  <div class="fr-card" id="frCard">
    ...
    <div class="fr-step" id="frStepMessage">
      ...
      <div class="fr-msg-chips" id="frMsgChips">
        <button class="fr-msg-chip fr-chip-expandable" data-expand="lingerie" onclick="frExpandChip(this,'lingerie')">Lingerie fina</button>
        ...
        <button class="fr-msg-chip fr-chip-expandable" data-expand="sensual" onclick="frExpandChip(this,'sensual')">Sensual discreto</button>
      </div>

      <!-- Sub-question: Lingerie cores -->
      <div class="fr-msg-subq" id="frSubqLingerie" style="display:none;">
        <div class="fr-msg-subq-title">Qual cor de lingerie você prefere?</div>
        <div class="fr-msg-chips fr-msg-chips-sub" id="frChipsLingerie">
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Preta</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Vermelha</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Rosa</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Branca</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Azul</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsLingerie')">Cores claras</button>
        </div>
        <button class="fr-msg-back-link" onclick="frCollapseChip('lingerie')">← Voltar às preferências</button>
      </div>

      <!-- Sub-question: Sensual discreto nível -->
      <div class="fr-msg-subq" id="frSubqSensual" style="display:none;">
        <div class="fr-msg-subq-title">Qual o seu nível de "sensual discreto"?</div>
        <div class="fr-msg-chips fr-msg-chips-sub" id="frChipsSensual">
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsSensual')">Toque leve de transparência</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsSensual')">Decote sutil</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsSensual')">Fendas</button>
          <button class="fr-msg-chip" onclick="frToggleSubChip(this,'frChipsSensual')">Lingerie por baixo</button>
        </div>
        <button class="fr-msg-back-link" onclick="frCollapseChip('sensual')">← Voltar às preferências</button>
      </div>

      <!-- Preferências de maquiagem -->
      <div id="frMakeupSection" style="display:none;">
        ...
        <div class="fr-msg-chips" id="frChipsMakeup">
          <button class="fr-msg-chip" onclick="frToggleSingleChip(this,'frChipsMakeup')">Sem maquiagem</button>
          <button class="fr-msg-chip" onclick="frToggleSingleChip(this,'frChipsMakeup')">Maquiagem leve</button>
          <button class="fr-msg-chip" onclick="frToggleSingleChip(this,'frChipsMakeup')">Produção completa</button>
        </div>
      </div>
...
<div class="fr-photo-overlay" id="frPhotoOverlay">
  ...
  <div class="fr-photo-stage">
    ...
    <div class="fr-photo-adding-label" id="frPhotoAddingLabel">Adicionando ao seu perfil...</div>
  </div>
...
function frShowStep(n) {
  ...
  // Clarity step tracking
  try { if(typeof clarity === 'function') clarity('event', 'step_view', {step: n}); } catch(e){}
}
...
function frShowPhotoPreview(url) {
  ...
  document.getElementById('frPhotoAddingLabel').classList.remove('show');
...
function frUsarEstaFoto() {
  ...
  const addingLabel = document.getElementById('frPhotoAddingLabel');
  // Show label in sync with animation start
  requestAnimationFrame(() => {
    prog.classList.add('animating');
    addingLabel.classList.add('show');
  });
  setTimeout(() => {
    // Fade out label in sync with end of animation
    addingLabel.classList.remove('show');
...
function frShowMessageStep() {
  ...
  if(s) s.classList.add('active');
  // Ensure main chips visible and makeup section visible
  const chips = document.getElementById('frMsgChips');
  if(chips) chips.style.display = 'flex';
  const makeup = document.getElementById('frMakeupSection');
  if(makeup) makeup.style.display = 'block';
  document.getElementById('frSubqLingerie').style.display = 'none';
  document.getElementById('frSubqSensual').style.display = 'none';
  window.scrollTo(0,0);
...
// Single-select chip (for makeup — only one at a time)
function frToggleSingleChip(el, groupId) {
  const group = document.getElementById(groupId);
  group.querySelectorAll('.fr-msg-chip').forEach(c => c.classList.remove('selected'));
  el.classList.add('selected');
}

// Sub-chip toggle (regular multi-select inside sub-question)
function frToggleSubChip(el, groupId) {
  el.classList.toggle('selected');
}

// Expand a chip into its sub-question panel
function frExpandChip(el, type) {
  const chips = document.getElementById('frMsgChips');
  const subq = document.getElementById('frSubq' + type.charAt(0).toUpperCase() + type.slice(1));
  const makeup = document.getElementById('frMakeupSection');

  // Hide all main chips and other sub-questions smoothly
  ...
  // Show targeted sub-question
  if(subq) subq.style.display = 'block';
}

// Collapse sub-question back to main chips
function frCollapseChip(type) {
  const chips = document.getElementById('frMsgChips');
  const subq = document.getElementById('frSubq' + type.charAt(0).toUpperCase() + type.slice(1));

  if(subq) subq.style.display = 'none';
  chips.style.display = 'flex';
  document.getElementById('frMakeupSection').style.display = 'block';

  // Keep chip visually selected if sub-chips were picked
  const expandBtn = chips.querySelector(`[data-expand="${type}"]`);
  const subChipsSelected = subq ? subq.querySelectorAll('.fr-msg-chip.selected').length > 0 : false;
  if(expandBtn) {
    if(subChipsSelected) expandBtn.classList.add('selected');
    else expandBtn.classList.remove('selected', 'expanded');
...
function frSendMessage() {
  ...
  const mainChips = Array.from(document.querySelectorAll('#frMsgChips .fr-msg-chip.selected')).map(c => c.textContent.trim());
  const lingerieChips = Array.from(document.querySelectorAll('#frChipsLingerie .fr-msg-chip.selected')).map(c => 'Lingerie fina: ' + c.textContent.trim());
  const sensualChips = Array.from(document.querySelectorAll('#frChipsSensual .fr-msg-chip.selected')).map(c => 'Sensual discreto: ' + c.textContent.trim());
  const makeupChips = Array.from(document.querySelectorAll('#frChipsMakeup .fr-msg-chip.selected')).map(c => 'Maquiagem: ' + c.textContent.trim());
  const allPrefs = [...mainChips, ...lingerieChips, ...sensualChips, ...makeupChips];

  frState.personalMessage = msg;
  frState.preferences = allPrefs;
  frSaveState();

  const hasSomething = msg.length > 0 || allPrefs.length > 0;
  if(hasSomething) {
    frShowToast('Sua mensagem foi enviada! ✓', 'success');
    setTimeout(() => frShowStep(7), 600);
  } else {
    frShowStep(7);
  }
}
