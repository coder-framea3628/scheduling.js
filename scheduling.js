...
.city-select::before {
  content: '';
  background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>') no-repeat center;
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step2" class="step">
      ...
      <label class="upload">
        ...
        <div class="upload-text" style="margin-bottom: 24px;">Foto de perfil (opcional)</div>
      </label>
      <div class="input-group">
    ...
    <div id="step5" class="step">
      ...
      <div class="comunicado-card">
        ...
        <div class="hotel-info">
          ...
          <p class="hotel-desc">Clique no botão 'Buscar cidade' e digite a cidade onde deseja a realização do seu encontro ou selecione 'Minha cidade não está na lista' e siga os passos indicados.</p>
        </div>
        <div class="hotel-actions">
          <button class="btn" onclick="showFaq()">Acessar a FAQ</button>
    ...
    <div id="step6" class="step">
      ...
      <h3 style="margin-bottom: 24px;">Inclua e aproveite</h3>
  ...
  <div class="date-fullscreen" id="date-fullscreen">
    ...
    <button class="load-more-dates" onclick="loadMoreDates()">Carregar mais datas</button>
    ...
    <div class="calendar-actions">
      <button class="btn secondary" id="clear-dates" style="width: 140px;">Limpar</button>
      <button class="btn" id="save-dates" style="width: 140px;">Confirmar data</button>
    </div>
  </div>
  ...
  <div class="payment-popup" id="payment-popup">
    ...
    <button class="btn" id="payment-done" style="display:none; background: var(--accent-light); color: #fff;">Pagamento efetuado</button>
  </div>
  <div class="faq-popup" id="faq-popup">
    <h2>Perguntas Frequentes</h2>
    <ul class="faq-list">
      <li><div class="faq-question">Como escolher hotel?</div><div class="faq-answer">Selecione na lista ou use custom.</div></li>
      <li><div class="faq-question">Proteção de dados?</div><div class="faq-answer">Conforme política de privacidade.</div></li>
      <li><div class="faq-question">O que é upsell?</div><div class="faq-answer">Adicionais para experiência.</div></li>
      <li><div class="faq-question">Reembolso?</div><div class="faq-answer">Veja política de reembolso.</div></li>
    </ul>
    <button class="btn" onclick="closeFaq()">Fechar</button>
  </div>
  <div class="help-sheet" id="help-sheet">
    <h2>Ajuda</h2>
    <ul class="help-list">
      <li onclick="window.open('https://t.me/suporteframebot', '_blank')">Suporte via Telegram</li>
      <li onclick="showFaqPopup()">FAQ</li>
      <li><a href="mailto:contato@frameag.com" style="color: var(--primary-color);">Email: contato@frameag.com</a></li>
    </ul>
  </div>
  <div class="overlay-black" id="overlay-black"></div>
  <div id="chatbot" onclick="showHelpSheet()">?</div>
`;
// ===== Lógica JavaScript =====
...
const today = new Date(2025, 10, 3); // November 3, 2025
...
function showHotelDetails(city, index) {
  ...
  content.innerHTML = `
    <img src="${hotel.largeImg}" class="popup-img" alt="${hotel.name}" style="margin: 0 auto;">
    <div style="text-align: left;">
      <h3 style="font-size: 1.2rem; font-weight: bold;">${hotel.name}</h3>
      <p style="font-size: 0.9rem;">${hotel.desc}</p>
      ...
      <button class="btn" style="position: sticky; bottom: 0;" onclick="selectHotel('${city}', ${index})">Selecionar hotel</button>
    </div>
  `;
...
confirmBtn.addEventListener('click', () => {
  ...
  showPaymentPopup(link);
});
function showPaymentPopup(link) {
  ...
  document.getElementById('payment-done').onclick = () => {
    ...
    showLoadingOverlay(['Verificando informações…', 'Analisando seus dados…', 'Tudo certo!'], () => showStep(7));
  };
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('step3-btn').addEventListener('click', () => { if (validateStep(3)) {
    ...
    showLoadingOverlay(['Verificando informações…', 'Analisando seus dados…', 'Tudo certo!'], () => showCalendar());
  } });
  ...
  document.getElementById('overlay-black').addEventListener('click', () => {
    document.querySelectorAll('.bottom-sheet.show, .payment-popup, .faq-popup, .warning-popup, .help-sheet.show').forEach(el => {
      if (el.id === 'payment-popup') closePaymentPopup();
      else if (el.id === 'faq-popup') closeFaq();
      else if (el.id === 'help-sheet') closeHelpSheet();
      else closeBottomSheet(el.id);
    });
  });
});
