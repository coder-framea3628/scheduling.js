...
.city-select input::before {
  ...
  width: 16px;
  height: 16px;
  background: url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0ibm9uZSIgc3Ryb2tlPSJ2YXIoLS1wcmltYXJ5LWNvbG9yKSIgc3Ryb2tlLXdpZHRoPSIyIj48Y2lyY2xlIGN4PSIxMSIgY3k9IjExIiByPSI4Ii8+PHBhdGggZD0iTTIxIDIxbC00LjM1LTQuMzUiLz48L3N2Zz4=') no-repeat center;
...
.upsell-card input:checked + label {
  border: 1px solid var(--primary-color);
}
.upsell-title {
  font-weight: 600;
...
.bottom-sheet.show ~ .container::before {
  content: '';
  position: fixed;
  top: 0;
  left: 0;
...
.hotel-popup .popup-content {
  flex-direction: column;
}
.hotel-popup .popup-img {
  max-height: 300px;
}
.hotel-popup .popup-content button {
  position: sticky;
  bottom: 0;
  width: 100%;
  background: white;
...
.explanatory .hotel-info {
  text-align: center;
}
.explanatory .hotel-img {
  height: 120px;
}
.tudo-incluso {
  background: #F4E8DB;
...
.step:not(#step1):not(#step2):not(#step3):not(#step4) p, .step:not(#step1):not(#step2):not(#step3):not(#step4) h2 {
  margin-bottom: 16px;
}
.step:not(#step1):not(#step2):not(#step3):not(#step4) .input-group {
  padding: 16px;
}
@media (max-width: 390px) {
  .container {
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step2" class="step">
      ...
      <label class="upload">
        ...
        <div class="circle-upload" id="profileCircle">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--primary-color)" stroke-width="2"><path d="M12 4v16m8-8H4"/></svg>
    ...
    <div id="step5" class="step">
      ...
      <div class="selected-summary show">
        ...
        <p id="selected-hotel">Hotel: Aguarde seleção</p>
      </div>
      ...
      <div class="hotel-card explanatory">
        <img class="hotel-img" src="https://framerusercontent.com/images/fkQ25pvFwNUzjrzPp50famYE.png" alt="Comunicado">
        <div class="hotel-info">
          <div class="hotel-name">A sua experiência Frame.</div>
          <p>Clique no botão 'Buscar cidade' e digite a cidade onde deseja a realização do seu encontro ou selecione 'Minha cidade não está na lista' e siga os passos indicados.</p>
        </div>
        <button class="btn" onclick="showBottomSheet('faq-popup')">Acessar a FAQ</button>
      </div>
    ...
    <div id="step6" class="step">
      ...
      <span class="tudo-incluso">✓ TUDO INCLUSO</span>
      <div class="hotel-header">
        <img id="hotel-photo" class="hotel-photo" src="" alt="Hotel">
        <div>
          <div class="hotel-name" id="resumo-hotel"></div>
          <div class="hotel-rating">5,0 (+900)<svg viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill="var(--primary-color)"/></svg></div>
        </div>
      </div>
      <div class="resumo-card">
        ...
        <div class="resumo-item"><span>Hóspedes:</span><span>2</span></div>
        <div class="resumo-item"><span>Tipo de atendimento:</span><span>Encontro presencial</span></div>
      </div>
  ...
  <div class="date-fullscreen" id="date-fullscreen">
    ...
    <div class="calendar-actions">
      ...
      <button class="btn" id="save-dates">Confirmar data</button>
    </div>
  </div>
  ...
  <div class="bottom-sheet" id="payment-instructions">
    <span class="close-popup">×</span>
    <h2>Instruções</h2>
    ...
    <button class="btn" id="go-to-payment">Ir para pagamento</button>
  </div>
  <div class="bottom-sheet" id="faq-popup">
    <span class="close-popup">×</span>
    <h2>Perguntas Frequentes</h2>
    <ul>
      <li><strong>Como funciona o agendamento?</strong> Você seleciona data, cidade e hotel, e prossegue para pagamento.</li>
      <li><strong>O que está incluso?</strong> Tudo para uma experiência completa, incluindo hospedagem e encontro.</li>
      <li><strong>Posso cancelar?</strong> Consulte a política de reembolso.</li>
      <li><strong>Meus dados estão seguros?</strong> Sim, protegidos pela nossa política de privacidade.</li>
    </ul>
  </div>
...
function selectDate(date) {
  if (!checkInDate) {
    ...
    checkInDate = date;
    checkOutDate = null;
    document.getElementById('date-message').textContent = 'Selecionou o novo Check-in';
  }
...
function initAutocomplete() {
  ...
  const explanatory = document.querySelector('.explanatory');
  ...
  input.addEventListener('input', () => {
    ...
    explanatory.style.display = 'none';
    const filtered = cities.filter(c => normalizeString(c.toLowerCase()).includes(val));
    filtered.forEach(c => {
      const option = document.createElement('div');
      option.className = 'city-option';
      option.innerHTML = `<svg class="pin" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>${c}`;
...
function loadHotels(city) {
    ...
    hotels[city].forEach((hotel, index) => {
      ...
      card.innerHTML = `
        <div class="hotel-img-container">
          ...
          <span class="hotel-favorite ${favorites.includes(city + index) ? 'favorited' : ''}" onclick="toggleFavorite(this, '${city}', ${index})">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
          </span>
        </div>
        <div class="hotel-info">
  ...
  document.getElementById('step5-btn').disabled = !selectedHotel;
  if (selectedHotel) {
...
function selectHotel(city, index, btn) {
  if (btn) {
    const text = btn.querySelector('span') || btn; // Se tiver span para texto
...
function showBottomSheet(id) {
  ...
  document.querySelector('.popup-overlay:not(.show), .bottom-sheet:not(.show)').addEventListener('click', closeAllPopups);
}
function closeBottomSheet(id) {
  document.getElementById(id).classList.remove('show');
  document.querySelector('.popup-overlay:not(.show), .bottom-sheet:not(.show)').removeEventListener('click', closeAllPopups);
}
function closeAllPopups(e) {
  if (e.target === this) {
    document.querySelectorAll('.popup-overlay.show, .bottom-sheet.show').forEach(el => el.classList.remove('show'));
  }
}
...
confirmBtn.addEventListener('click', () => {
  ...
  showBottomSheet('payment-instructions');
  document.getElementById('go-to-payment').addEventListener('click', () => {
    window.open(link, '_blank');
    closeBottomSheet('payment-instructions');
    confirmBtn.textContent = 'Pagamento efetuado';
    confirmBtn.style.background = '#ff8787';
    confirmBtn.style.color = 'white';
    confirmBtn.onclick = () => {
      showToast('O pagamento será verificado junto com os dados.');
      showLoading();
      setTimeout(() => {
        hideLoading();
        showStep(7);
...
function showSuccessOverlay(messages, callback) {
  showLoading();
  let i = 0;
  const interval = setInterval(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('save-dates').addEventListener('click', () => {
    if (checkInDate && checkOutDate) {
      ...
      showSuccessOverlay(['Salvando as informações...', 'Agendando...'], () => {
  ...
  document.getElementById('step4-btn').addEventListener('click', () => { if (validateStep(4)) {
    showSuccessOverlay(['Verificando informações…', 'Analisando seus dados…', 'Tudo certo!'], showCalendar);
  } });
  document.getElementById('step5-btn').addEventListener('click', () => { if (validateStep(5)) showStep(6); });
  document.getElementById('step7-btn').addEventListener('click', () => window.location.href = 'https://frameag.com/kyc/demo');
  ...
  if (userData.customOption === 'self') {
    ...
    document.getElementById('step7').appendChild(reminder);
  }
  ...
  if (userData.personal) {
    document.getElementById('nome').value = userData.personal.nome;
    document.getElementById('cpf').value = userData.personal.cpf;
    document.getElementById('rua').value = userData.personal.rua;
    document.getElementById('bairro').value = userData.personal.bairro;
    document.getElementById('numero').value = userData.personal.numero;
    document.getElementById('cidade').value = userData.personal.cidade;
    document.getElementById('estado').value = userData.personal.estado;
  }
  if (userData.profile) {
    document.getElementById('displayName').value = userData.profile.displayName;
    document.getElementById('showAge').value = userData.profile.showAge;
  }
  if (userData.docType) {
    document.getElementById('docType').value = userData.docType;
  }
  if (checkInDate && checkOutDate) {
    document.getElementById('selected-dates').textContent = `Data escolhida: ${checkInDate.toLocaleDateString('pt-BR')} - ${checkOutDate.toLocaleDateString('pt-BR')}`;
  }
  ...
  document.getElementById('resumo-dates').textContent = `${checkInDate ? checkInDate.toLocaleDateString('pt-BR') : ''} - ${checkOutDate ? checkOutDate.toLocaleDateString('pt-BR') : ''}`;
  document.getElementById('resumo-hotel').textContent = selectedHotel ? selectedHotel.name : '';
  document.getElementById('hotel-photo').src = selectedHotel ? selectedHotel.profileImg : '';
});
