...
.fr-kyc-logo {
  ...
  margin: 0 auto 25px;
  width: 100%;
  max-width: 260px;        /* em desktop fica ~260px */
  height: auto;
}

/* Em telas bem pequenas (mobile) fica um pouco menor ainda */
...
.fr-kyc-big-instr.visible { opacity: 1; }
.fr-kyc-big-instr-icon { font-size: 42px; margin-bottom: 10px; line-height: 1; }
.fr-kyc-review-ov {
  position: absolute;
  inset: 0;
  display: none;
...
.fr-kyc-review-ov.show { display: block; }
.fr-kyc-review-ov img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  transform: scaleX(-1);
...
.fr-kyc-action-btn.primary { background: var(--fr-bronze); color: #fff; box-shadow: 0 4px 14px rgba(172,134,92,0.3); }
.fr-kyc-action-btn.primary:hover { background: var(--fr-bronze-mid); transform: translateY(-1px); }
.fr-kyc-action-btn.secondary { background: var(--fr-gray-light); color: var(--fr-charcoal); }
.fr-kyc-action-btn.secondary:hover { background: #ddd; }
.fr-kyc-start-btn {
  display: none;
  width: 100%;
  padding: 15px;
  ...
  cursor: pointer;
  margin-top: 14px;
  box-shadow: 0 4px 18px rgba(172,134,92,0.4);
  transition: all var(--fr-transition);
  align-items: center;
  justify-content: center;
  gap: 8px;
}
.fr-kyc-start-btn:hover { background: var(--fr-bronze-mid); transform: translateY(-1px); }
.fr-kyc-start-btn:active { transform: scale(0.98); }
.fr-kyc-review-info {
  display: none;
  text-align: center;
  margin-top: 4px;
}
.fr-kyc-review-info .fr-kyc-status-top { margin-top: 0; }
...
<div class="fr-kyc-overlay" id="frKycOverlay">
  <div class="fr-kyc-box">
    ...
    <div class="fr-kyc-camera-wrap">
      ...
      <canvas id="frKycCanvas" style="display:none;"></canvas>
      ...
      <!-- Big instruction overlay for head movements -->
      <div class="fr-kyc-big-instr" id="frKycBigInstr">
        <div class="fr-kyc-big-instr-icon" id="frKycBigInstrIcon"></div>
        <div id="frKycBigInstrText"></div>
      </div>
      <!-- Review overlay: shows captured photo -->
      <div class="fr-kyc-review-ov" id="frKycReviewOv">
        <img id="frKycReviewImg" alt="Foto capturada">
      </div>
    ...
    <!-- Default status (shown during init and processing) -->
    ...
    <!-- Start capture button (visible when camera is ready) -->
    <button class="fr-kyc-start-btn" id="frKycStartCapBtn" onclick="frKycStartCapture()">
      ${ICONS.camera} Começar verificação
    </button>
    <!-- Review actions (visible after photo is captured) -->
    <div class="fr-kyc-review-info" id="frKycReviewInfo">
      <div class="fr-kyc-status-top">A foto ficou boa?</div>
      <div class="fr-kyc-status-bottom">Certifique-se de que seu rosto está nítido e sem reflexos.</div>
      <div class="fr-kyc-action-row">
        <button class="fr-kyc-action-btn secondary" onclick="frKycRetake()">Tirar novamente</button>
        <button class="fr-kyc-action-btn primary" onclick="frKycConfirmPhoto()">Enviar foto</button>
      </div>
    </div>
  </div>
</div>
...
function frInitCamera() {
  ...
  document.getElementById('frKycStartCapBtn').style.display = 'none';
  document.getElementById('frKycReviewInfo').style.display = 'none';
  document.getElementById('frKycReviewOv').classList.remove('show');
  document.getElementById('frKycSuccessOverlay').classList.remove('show');
  document.getElementById('frKycCountdown').classList.remove('show');
  document.getElementById('frKycScanLine').classList.remove('active');
  document.getElementById('frKycOval').classList.remove('scanning');
  document.getElementById('frKycBigInstr').classList.remove('visible');
  document.getElementById('frKycMsg').textContent = 'Centralize seu rosto no oval';
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user', width: { ideal: 1280 }, height: { ideal: 720 } } })
    .then(stream => {
      ...
      vid.onloadedmetadata = () => {
        ...
        document.getElementById('frKycSpinner').classList.remove('show');
        frKycSetReady();
      };
      // Fallback if onloadedmetadata never fires
      setTimeout(() => {
        if (document.getElementById('frKycSpinner').classList.contains('show')) {
          frSetBtnLoading('frKycStartBtn', false);
          document.getElementById('frKycSpinner').classList.remove('show');
          frKycSetReady();
        }
    ...
    .catch(err => {
      ...
      frSetBtnLoading('frKycStartBtn', false);
      document.getElementById('frKycSpinner').classList.remove('show');
      // Proceed visually even without camera access
      frKycSetReady();
    });
}

// Phase: Camera is live, waiting for user to click "Começar"
function frKycSetReady() {
  document.getElementById('frKycStatusTop').textContent = 'Câmera ativa';
  document.getElementById('frKycStatusBottom').textContent = 'Posicione seu rosto no oval e clique em Começar';
  document.getElementById('frKycMsg').textContent = 'Posicione seu rosto';
  const btn = document.getElementById('frKycStartCapBtn');
  btn.style.display = 'flex';
}

// User clicked "Começar" — start countdown then instructions
function frKycStartCapture() {
  document.getElementById('frKycStartCapBtn').style.display = 'none';
  document.getElementById('frKycStatusTop').textContent = 'Prepare-se...';
  document.getElementById('frKycStatusBottom').textContent = 'Siga as instruções na tela';
  document.getElementById('frKycMsg').textContent = 'Preparando...';
  frKycCountdown(3, frKycRunInstructions);
}

// Countdown 3-2-1 then callback
function frKycCountdown(from, callback) {
...
// Show a big instruction with icon, duration ms, then resolve
function frKycShowInstruction(text, icon, duration) {
  return new Promise(resolve => {
    const instr = document.getElementById('frKycBigInstr');
    document.getElementById('frKycBigInstrIcon').textContent = icon || '';
    document.getElementById('frKycBigInstrText').textContent = text;
    instr.classList.add('visible');
    setTimeout(() => {
      instr.classList.remove('visible');
      setTimeout(resolve, 350); // wait for fade-out
    }, duration);
  });
}

// Run the head-movement instruction sequence then capture
async function frKycRunInstructions() {
  document.getElementById('frKycScanLine').classList.add('active');
  document.getElementById('frKycOval').classList.add('scanning');
  document.getElementById('frKycMsg').textContent = 'Siga as instruções';
  document.getElementById('frKycStatusTop').textContent = 'Verificação em andamento...';
  document.getElementById('frKycStatusBottom').textContent = 'Siga as instruções na tela';

  await frKycShowInstruction('Mova sua cabeça para a esquerda', '👈', 2500);
  await frKycShowInstruction('Mova sua cabeça para a direita', '👉', 2500);
  await frKycShowInstruction('Olhe para frente', '😐', 2000);
  await frKycShowInstruction('Mantenha-se parado', '✋', 2000);

  document.getElementById('frKycScanLine').classList.remove('active');
  document.getElementById('frKycOval').classList.remove('scanning');
  document.getElementById('frKycMsg').textContent = 'Capturando foto...';
...
// Capture a frame from the video into a canvas
function frKycTakePicture() {
  const vid = document.getElementById('frKycVideo');
  const canvas = document.getElementById('frKycCanvas');
  const ctx = canvas.getContext('2d');

  if (vid.videoWidth && vid.videoHeight) {
    canvas.width = vid.videoWidth;
    canvas.height = vid.videoHeight;
    // Draw mirrored (as seen on screen)
    ctx.save();
    ctx.scale(-1, 1);
    ctx.drawImage(vid, -canvas.width, 0);
    ctx.restore();
  } else {
    // Fallback solid frame (no camera available)
    canvas.width = 640;
    canvas.height = 480;
  ...
  frKycShowReview(dataUrl);
}

// Show the captured photo for user review
function frKycShowReview(dataUrl) {
  document.getElementById('frKycReviewImg').src = dataUrl;
  document.getElementById('frKycReviewOv').classList.add('show');
  document.getElementById('frKycMsg').textContent = '';
  document.getElementById('frKycStatusTop').style.display = 'none';
  document.getElementById('frKycStatusBottom').style.display = 'none';

  const reviewInfo = document.getElementById('frKycReviewInfo');
  reviewInfo.style.display = 'block';
}

// User wants to retake — reset to ready state
function frKycRetake() {
  document.getElementById('frKycReviewOv').classList.remove('show');
  document.getElementById('frKycReviewInfo').style.display = 'none';
  document.getElementById('frKycStatusTop').style.display = '';
  document.getElementById('frKycStatusBottom').style.display = '';
  document.getElementById('frKycMsg').textContent = 'Posicione seu rosto';
  frKycSetReady();
}

// User confirmed photo — run processing then success
function frKycConfirmPhoto() {
  document.getElementById('frKycReviewOv').classList.remove('show');
  document.getElementById('frKycReviewInfo').style.display = 'none';
  document.getElementById('frKycStatusTop').style.display = '';
  document.getElementById('frKycStatusBottom').style.display = '';
...
// Processing sequence — always ends in success
function frKycRunSequence() {
  ...
  statusTop.textContent = 'Validando sua biometria...';
  statusBot.textContent = 'Aguarde enquanto analisamos seus dados';
  msg.textContent = 'Processando dados...';

  setTimeout(() => {
    statusTop.textContent = 'Confirmando identidade...';
    statusBot.textContent = 'Verificando padrões faciais';
    msg.textContent = 'Analisando biometria...';

    setTimeout(() => {
      statusTop.textContent = 'Dados protegidos com segurança Frame...';
      statusBot.textContent = 'Última etapa...';
      msg.textContent = 'Finalizando verificação...';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const s5btn = document.getElementById('frStep5Btn');
  if(s5btn) s5btn.addEventListener('click', frStep5Next);
