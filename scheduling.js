...
function frSelectCalDay(dateStr) {
  if(frState.calMode === 'checkin') {
    ...
    document.getElementById('frCalSubtitle').textContent = 'Check-out preenchido de forma automática. Você pode ajustar para até 2 dias.';
  } else {
    if(!frState.checkIn || dateStr <= frState.checkIn) {
      frShowToast('Seu check-out só pode ser realizado após o check-in.', 'error');
      return;
    }
    // Max 2 days
