...
.city-select::before {
  ...
  width: 24px;
  height: 24px;
  background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>') no-repeat center / 24px;
}
#city-list {
  position: absolute;
  background: var(--secondary-bg);
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step1" class="step active">
      ...
      <div class="input-group">
        ...
        <div class="tooltip">Nome completo até 100 caracteres.</div>
      ...
      <div class="input-group">
        ...
        <div class="tooltip">Endereço completo até 100 caracteres.</div>
        <div class="error-message"></div>
      </div>
      <div class="row">
        <div class="input-group">
          ...
          <div class="tooltip">Bairro até 100 caracteres.</div>
          <div class="error-message"></div>
        </div>
        <div class="input-group">
    ...
    <div id="step2" class="step">
      ...
      <div class="input-group">
        ...
        <div class="tooltip">Nome de exibição até 100 caracteres.</div>
        <div class="error-message"></div>
      </div>
      <div class="input-group">
    ...
    <div id="step5" class="step">
      ...
      <div class="comunicado-card">
        ...
        <div class="hotel-actions">
          <button class="btn" onclick="document.getElementById('faq-popup').style.display = 'block'; document.getElementById('overlay-black').style.display = 'block';">Acessar FAQ</button>
        </div>
      </div>
      ...
      <div id="hotels-section">
        <h3 id="hotels-title" style="font-size: 1rem;"></h3>
        <div id="hotel-list" class="hotel-list"></div>
      </div>
      <button class="btn" id="step5-btn" style="display: none;" disabled>Prosseguir</button>
    </div>
    <div id="step6" class="step">
      ...
      <div class="resumo-card">
        ...
        <div class="resumo-item"><span>Data: ${checkInDate.toLocaleDateString('pt-BR')} - ${checkOutDate.toLocaleDateString('pt-BR')}</span><span class="edit-link" onclick="editDates()">Editar</span></div>
      </div>
  ...
  <div class="faq-popup" id="faq-popup">
    ...
    <button class="btn" onclick="document.getElementById('faq-popup').style.display = 'none'; document.getElementById('overlay-black').style.display = 'none';">Fechar</button>
  </div>
  <div class="overlay-white" id="premium-overlay">
    <div class="premium-container">
      ...
      <div class="premium-sub">Por ser um usuário Premium, você conta com hospedagem inclusa no seu agendamento</div>
      <p>Deseja aproveitar esta oferta?</p>
      <button class="btn premium-btn" onclick="closePremiumOverlay(); showStep(2);">Sim, seguir com a oferta</button>
      <div class="premium-no" onclick="closePremiumOverlay(); showStep(2);">Seguir sem a oferta</div>
    </div>
  </div>
...
let checkInDate = localStorage.getItem('dates') ? JSON.parse(localStorage.getItem('dates')).checkIn : null;
let checkOutDate = localStorage.getItem('dates') ? JSON.parse(localStorage.getItem('dates')).checkOut : null;
if (checkInDate) checkInDate = new Date(checkInDate);
if (checkOutDate) checkOutDate = new Date(checkOutDate);
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (valid) {
      ...
      localStorage.setItem('dates', JSON.stringify({checkIn: checkInDate.toISOString(), checkOut: checkOutDate.toISOString()}));
...
function renderCalendar(month, year) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (checkInDate && checkInDate.getTime() === dayDate.getTime()) {
      ...
      day.style.opacity = (checkInDate || checkOutDate) ? 0.7 : 1;
    }
...
function showHotelsSection(cidade) {
  document.getElementById('search-section').style.display = 'none';
  document.getElementById('hotels-section').style.display = 'block';
  document.getElementById('hotels-title').textContent = `Hotéis parceiros em: ${cidade}`;
}
...
function showHotelDetails(city, index) {
  ...
  content.innerHTML = `
    ...
    <p style="text-align: left;">${hotel.desc}</p>
