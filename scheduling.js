...
/* === TOOLTIP === */
.fr-tooltip-wrap { position: relative; display: inline-block; width: 100%; }
.fr-tooltip-bubble {
  position: absolute;
  bottom: calc(100% + 8px);
  left: 0;
...
.fr-tooltip-bubble::after {
  content: '';
  position: absolute;
  top: 100%;
  left: 16px;
  border: 6px solid transparent;
  border-top-color: var(--fr-charcoal);
}
.fr-tooltip-bubble.visible { opacity: 1; transform: translateY(0); }

/* === HOTEL BOTTOM SHEET === */
.fr-hotel-bottom-sheet {
  position: fixed;
  bottom: 0; left: 0; right: 0;
  background: var(--fr-white);
  border-top: 1.5px solid var(--fr-gray-light);
...
.fr-hotel-bottom-sheet.show { transform: translateY(0); }
.fr-hotel-bs-info { flex: 1; min-width: 0; }
.fr-hotel-bs-name {
  font-family: 'Montserrat', sans-serif;
  font-weight: 700;
  font-size: 0.9rem;
...
.fr-hotel-bs-date { font-size: 0.75rem; color: var(--fr-gray); margin-top: 2px; }
.fr-hotel-bs-btn {
  padding: 13px 22px;
  border-radius: var(--fr-radius-pill);
  background: var(--fr-bronze);
  ...
  cursor: pointer;
  white-space: nowrap;
  box-shadow: 0 4px 16px rgba(172,134,92,0.35);
  transition: all var(--fr-transition);
  flex-shrink: 0;
}
.fr-hotel-bs-btn:hover { background: var(--fr-bronze-mid); }
...
.fr-photo-sheet.show { transform: translateY(0); }
.fr-photo-sheet-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 16px 20px;
...
.fr-photo-sheet-title { font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 1rem; color: #fff; }
.fr-photo-tips-btn {
  background: none; border: none; color: rgba(255,255,255,0.75); font-family: 'Poppins', sans-serif;
  font-size: 0.875rem; font-weight: 500; cursor: pointer; padding: 6px 12px;
  border-radius: 20px; transition: background 0.2s;
}
.fr-photo-tips-btn:hover { background: rgba(255,255,255,0.1); }
.fr-photo-sheet-body {
  flex: 1;
  display: flex;
  flex-direction: column;
...
.fr-photo-info-box-title { font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 0.95rem; color: #fff; margin-bottom: 6px; }
.fr-photo-info-box-sub { font-size: 0.8rem; color: rgba(255,255,255,0.7); line-height: 1.5; }
.fr-photo-change-link {
  color: rgba(255,255,255,0.6); font-size: 0.8rem; text-decoration: underline;
  background: none; border: none; cursor: pointer; margin-top: 10px; font-family: 'Poppins', sans-serif;
  display: block; text-align: left; padding: 0;
}
.fr-photo-sheet-footer {
...
.fr-photo-edit-btn {
  background: none; border: none; color: rgba(255,255,255,0.7); font-family: 'Poppins', sans-serif;
...
.fr-photo-use-btn {
  ...
  max-width: 200px;
  padding: 14px 20px;
  background: linear-gradient(110deg, #E8538A, #C0392B);
...
.fr-photo-use-btn:hover { opacity: 0.9; }
.fr-photo-dots {
  display: flex; gap: 5px; align-items: center; justify-content: center;
}
.fr-photo-dot {
  width: 8px; height: 8px; border-radius: 50%; background: rgba(255,255,255,0.9);
  animation: frDotBounce 0.9s ease-in-out infinite;
}
.fr-photo-dot:nth-child(2) { animation-delay: 0.15s; }
.fr-photo-dot:nth-child(3) { animation-delay: 0.3s; }
@keyframes frDotBounce {
...
.fr-photo-tips-modal.show { display: flex; }
.fr-photo-tips-close {
  position: absolute;
  top: 16px; right: 16px;
...
.fr-photo-tips-close:hover { background: rgba(255,255,255,0.25); }
.fr-photo-tips-content { background: rgba(30,30,30,0.98); border-radius: 20px; padding: 24px; width: 100%; max-width: 340px; }
.fr-photo-tips-title { font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 1rem; color: #fff; margin-bottom: 12px; text-align: center; }
.fr-photo-tips-list { list-style: none; padding: 0; margin: 0; }
.fr-photo-tips-list li { color: rgba(255,255,255,0.85); font-size: 0.84rem; padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.08); display: flex; align-items: center; gap: 10px; line-height: 1.4; }
.fr-photo-tips-list li:last-child { border-bottom: none; }
.fr-photo-tips-list li::before { content: '•'; color: #E8538A; font-size: 1.2rem; flex-shrink: 0; }
...
.fr-resume-title { font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 1.15rem; margin-bottom: 6px; }
.fr-resume-sub { font-size: 0.855rem; color: var(--fr-gray); margin-bottom: 22px; line-height: 1.5; }
...
.fr-support-btn:hover { background: var(--fr-bronze); transform: scale(1.08); }
...
.fr-custom-city-btn:hover { background: var(--fr-bronze-pale); }
...
/* === KYC SUCCESS SCREEN === */
.fr-kyc-success-screen {
  display: none;
  padding: 24px 28px 32px;
  animation: frFadeIn 0.4s ease both;
  text-align: center;
}
.fr-kyc-success-screen.active { display: block; }
.fr-kyc-ss-icon {
  width: 80px; height: 80px;
  background: linear-gradient(135deg, #2E7D5C, #3da574);
  border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
...
/* === PULSE SLOW === */
@keyframes frPulseSlow {
  0%,100% { box-shadow: 0 0 0 0 rgba(172,134,92,0.3); }
  50% { box-shadow: 0 0 0 8px rgba(172,134,92,0); }
}
.fr-incluso-badge { animation: frPulseSlow 3.5s ease-in-out infinite !important; }

/* === PREMIUM DARK === */
.fr-premium-overlay.dark-mode {
  background: rgba(10,10,12,0.96);
}
.fr-premium-box.dark-mode {
  background: linear-gradient(145deg, #1a1a2e 0%, #16213e 50%, #1a1a2e 100%);
  border: 1px solid rgba(172,134,92,0.3);
  box-shadow: 0 20px 60px rgba(0,0,0,0.5), 0 0 40px rgba(172,134,92,0.08);
}
.fr-premium-box.dark-mode .fr-h2 { color: #fff; }
.fr-premium-box.dark-mode p { color: rgba(255,255,255,0.65); }
.fr-premium-badge.dark-mode {
  background: linear-gradient(110deg, rgba(172,134,92,0.2), rgba(210,159,102,0.2));
  color: var(--fr-bronze-light);
  border: 1px solid rgba(172,134,92,0.35);
}
.fr-premium-logo.dark-mode { filter: brightness(2) saturate(0); }
...
/* === CALENDAR LOCKED DAY === */
.fr-cal-day.locked { color: var(--fr-gray-light); cursor: not-allowed; pointer-events: auto; }
`;

document.head.appendChild(frStyle);

// ===== CONSTANTS =====
const FR_PROHIBITED = ['cu','pau','ppk','goza','chupa','puta','kids','kid','baby','bebe','cp','porno','estupro','estuprador','buceta','caralho','foder','fode','transar','sexo','bucetinha','roubar','exterminar','cuzinho','chupetinha','viado','baitola','prostituta','pedofilo','bct','nuds','leitada','fetiche','penis','vagina','pepeka','piroca','gozada','pedofilia','violencia','ameaca','drogas','ilegal','merda','porra','otario','idiota','imbecil','retardado','maldito','safado','vadia','piranha','vagabundo','assassino','matar','cocaina','maconha','heroina','crack','lsd','mdma','ecstasy','anfetamina','metanfetamina','louco','doido','corno','fdp','desgraça','arrombado','lixo','inutil','macaco','negro','preto','gay','bicha','travesti','trans','gordo','gorda','feio','feia','fuck','shit','asshole','bastard','whore','slut','bitch','dick','pussy','cunt','nigger','faggot','rape','kill','murder','terrorismo','terrorista','bomba','explosivo','arma','pistola','fuzil','metralhadora'];
...
function frValidateCpfAlgorithm(cpf) {
  ...
  r = (sum * 10) % 11;
  if(r === 10 || r === 11) r = 0;
  if(r !== parseInt(nums[10])) return false;
  return true;
}
...
// Mask sensitive data for localStorage
function frMaskData(state) {
  const s = JSON.parse(JSON.stringify(state));
  if(s.userData && s.userData.cpf) {
    const c = s.userData.cpf.replace(/\D/g,'');
    s.userData.cpf = c.slice(0,6).replace(/(\d{3})(\d{3})/,'$1.$2') + '.*-**';
  }
  if(s.userData && s.userData.nome) {
    const parts = s.userData.nome.split(' ');
    s.userData.nome = parts[0] + (parts.length > 1 ? ' ' + parts.slice(1).map(p => p[0]+'***').join(' ') : '');
  }
  if(s.userData && s.userData.rua) s.userData.rua = '***';
  if(s.userData && s.userData.bairro) s.userData.bairro = '***';
  return s;
}
...
try {
  ...
  if(s) {
    ...
    const now = Date.now();
    const maxAge = p.completed ? 7*24*60*60*1000 : 1*24*60*60*1000;
    if(p.savedAt && (now - p.savedAt) < maxAge) {
      // Only restore step (not sensitive masked data)
      frState.step = p.step || 1;
      frState.docType = p.docType || null;
      frState.hotelCity = p.hotelCity || null;
      frState.hotel = p.hotel || null;
      frState.checkIn = p.checkIn || null;
      frState.checkOut = p.checkOut || null;
      frState.upsell1 = p.upsell1 || false;
      frState.upsell2 = p.upsell2 || false;
      frState.warningCount = p.warningCount || 0;
      frState.completed = p.completed || false;
    } else {
...
const frSaveState = () => {
  try {
    ...
    const masked = frMaskData(frState);
    localStorage.setItem('fr_sched_state', JSON.stringify(masked));
  } catch(e){}
};
...
<div class="fr-wrap">
  <div class="fr-card" id="frCard">
    <div class="fr-progress-wrap">
      <div class="fr-progress-track"><div class="fr-progress-fill" id="frProgress" role="progressbar" aria-valuenow="1" aria-valuemin="1" aria-valuemax="8"></div></div>
    ...
    <div class="fr-step active" id="frStep1">
      ...
      <div class="fr-field">
        ...
        <div class="fr-tooltip-wrap">
          <input class="fr-input" type="text" id="frNome" placeholder="Ex: João da Silva Oliveira" autocomplete="name" aria-required="true">
          <div class="fr-tooltip-bubble" id="frNomeTooltip">Digite seu nome completo exatamente como está nos seus documentos.</div>
        </div>
      ...
      <div class="fr-field">
        ...
        <div class="fr-tooltip-wrap">
          ...
          <div class="fr-tooltip-bubble" id="frCpfTooltip">Dados corretos garantem a não reprovação do agendamento.</div>
        </div>
        <div class="fr-err" id="frCpfErr"></div>
      </div>
      <div class="fr-field">
        <label class="fr-label" for="frRua">Rua / Avenida</label>
        <input class="fr-input" type="text" id="frRua" placeholder="Ex: Rua das Flores" autocomplete="street-address" aria-required="true">
    ...
    <div class="fr-step" id="frStep2">
      ...
      <div class="fr-profile-center">
        <label class="fr-profile-upload" id="frProfileLabel" for="frProfilePic">
          <input type="file" id="frProfilePic" accept="image/jpeg,image/png,image/webp">
          <img id="frProfilePreview" src="" alt="Foto">
          ${ICONS.person}
        </label>
        <div class="fr-profile-center-label">Foto de perfil (opcional)</div>
      </div>
      <div class="fr-field">
        <label class="fr-label" for="frDisplayName">Nome de exibição</label>
        <div class="fr-tooltip-wrap">
          <input class="fr-input" type="text" id="frDisplayName" placeholder="Como prefere ser chamado(a)" aria-required="true">
          <div class="fr-tooltip-bubble" id="frDisplayNameTooltip">Se preferir, nessa etapa você pode optar por não utilizar/exibir seu nome real para a modelo.</div>
        </div>
        <div class="fr-err" id="frDisplayNameErr"></div>
      </div>
      <div class="fr-field">
        <label class="fr-label" for="frShowAge">Exibir sua idade para a modelo?</label>
        <select class="fr-select" id="frShowAge" aria-required="true">
          <option value="" disabled selected>Selecione</option>
          <option value="yes">Sim, exibir minha idade</option>
          <option value="no">Não exibir minha idade</option>
        </select>
        <div class="fr-age-note">
          <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;color:var(--fr-bronze)"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
          <span>Nota: A idade será exibida conforme seus documentos.</span>
        </div>
      </div>
    ...
    <div class="fr-step" id="frStep3">
      ...
      <div class="fr-doc-privacy">
        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;margin-top:1px;"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
        Nós usamos criptografia de ponta a ponta. Seus documentos são deletados automaticamente após aprovação.
      </div>
    ...
    <div class="fr-step" id="frStep5">
      ...
      <div id="frHotelSection" style="display:none;">
        ...
        <button class="fr-custom-city-btn" onclick="frShowCustomCity()" id="frCustomCityBtn">
    ...
    <!-- Hotel bottom sheet -->
    <div class="fr-hotel-bottom-sheet" id="frHotelBottomSheet">
      <div class="fr-hotel-bs-info">
        <div class="fr-hotel-bs-name" id="frHotelBsName">—</div>
        <div class="fr-hotel-bs-date" id="frHotelBsDate"></div>
      </div>
      <button class="fr-hotel-bs-btn" onclick="frStep5Next()" aria-label="Confirmar reserva">Confirmar reserva</button>
    </div>
    ...
    <!-- STEP 9: KYC Success -->
    <div class="fr-step" id="frStep9">
      <div class="fr-kyc-ss-icon">
      ...
      <div class="fr-h1" style="text-align:center;margin-bottom:8px;">Identidade confirmada!</div>
      <div class="fr-kyc-ss-hotel" id="frKycSsHotel"></div>
      <p class="fr-sub" style="text-align:center;">Sua reserva no <strong id="frKycSsHotelName">hotel</strong> está sendo confirmada.<br>Você receberá o código de reserva e detalhes por e-mail assim que a análise de dados terminar.</p>
      <div style="font-size:0.875rem;color:var(--fr-charcoal);text-align:center;margin:20px 0 14px;font-weight:500;">Para concluir clique abaixo:</div>
      <a href="https://www.frameag.com/kyc/success" class="fr-btn fr-btn-gradient" style="text-decoration:none;display:flex;align-items:center;justify-content:center;gap:8px;">
        <span>Finalizar envio de dados</span>
...
<div class="fr-cal-overlay" id="frCalOverlay">
  <div class="fr-cal-header">
    ...
    <button class="fr-cal-close" onclick="frCloseCalendar()" aria-label="Fechar calendário">${ICONS.close}</button>
  </div>
  <div class="fr-cal-subtitle" id="frCalSubtitle">Selecione a data de sua hospedagem com a modelo</div>
...
<!-- Premium overlay -->
<div class="fr-premium-overlay dark-mode" id="frPremiumOverlay">
  <div class="fr-premium-box dark-mode">
    <img class="fr-premium-logo dark-mode" src="https://framerusercontent.com/images/yVa3ycC9y0QeJOv3SFDDzBO1WYE.png" alt="Frame">
    <div class="fr-premium-badge dark-mode"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5.636 16C2.91 16 2 14 2 12s.91-4 3.636-4c3.637 0 9.091 8 12.728 8C21.09 16 22 14 22 12s-.91-4-3.636-4c-3.637 0-9.091 8-12.728 8z"/></svg> Membro Premium</div>
    <div class="fr-h2" style="margin-bottom:8px;">Identificamos sua assinatura Premium.</div>
    <p style="font-size:0.875rem;margin-bottom:20px;">Por ser um usuário Premium, você conta com hospedagem inclusa no seu agendamento — sem custo adicional.</p>
    <button class="fr-btn fr-btn-primary" onclick="frAcceptPremium()">
      <span class="fr-btn-label">Seguir com a oferta</span>
    </button>
    <button class="fr-btn-text" onclick="frDeclinePremium()" style="color:rgba(255,255,255,0.5);">Rejeitar oferta</button>
  </div>
</div>
...
<div class="fr-resume-overlay" id="frResumeOverlay">
  <div class="fr-resume-box">
    <div class="fr-resume-icon">⏱</div>
    <div class="fr-resume-title">Continuar de onde parou</div>
    <p class="fr-resume-sub">Encontramos um agendamento em andamento. Deseja continuar de onde você parou?</p>
    <button class="fr-btn fr-btn-primary" onclick="frResumeProgress()" style="margin-bottom:10px;">
      <span class="fr-btn-label">Continuar agendamento</span>
    </button>
    <button class="fr-btn fr-btn-ghost" onclick="frResetProgress()">
      <span class="fr-btn-label">Começar do zero</span>
    </button>
  </div>
</div>

<!-- Support button -->
<button class="fr-support-btn" onclick="frOpenSupport()" aria-label="Falar com suporte" title="Falar com suporte">
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
</button>

<!-- Photo preview bottom sheet -->
<div class="fr-photo-sheet" id="frPhotoSheet">
  <div class="fr-photo-sheet-header">
    <div class="fr-photo-sheet-title">Veja como vai ficar</div>
    <button class="fr-photo-tips-btn" onclick="frTogglePhotoTips()" id="frPhotoTipsToggle">Dicas</button>
  </div>
  <div class="fr-photo-sheet-body">
    <div class="fr-photo-circle-wrap" id="frPhotoCircleWrap">
      <img id="frPhotoSheetImg" src="" alt="Foto de perfil">
      <canvas class="fr-photo-arc-canvas" id="frPhotoArcCanvas"></canvas>
    </div>
    <div class="fr-photo-info-box">
      <div class="fr-photo-info-box-title">Verifique sua foto</div>
      <div class="fr-photo-info-box-sub">Essa imagem será exibida apenas para a modelo reconhecer você no hotel e é opcional</div>
      <button class="fr-photo-change-link" onclick="frPhotoChangeImage()">Escolher uma nova imagem</button>
    </div>
    <div class="fr-photo-tips-modal" id="frPhotoTipsModal">
      <button class="fr-photo-tips-close" onclick="frTogglePhotoTips()">×</button>
      <div class="fr-photo-tips-content">
        <div class="fr-photo-tips-title">Dicas de fotografia</div>
        <ul class="fr-photo-tips-list">
          <li>Seu rosto deve estar bem iluminado, nítido e centralizado no enquadramento.</li>
          <li>Olhe diretamente para a câmera e seja a única pessoa na foto.</li>
          <li>Use uma foto de rosto, não de animal ou paisagem.</li>
          <li>Evite fotos com filtros, óculos escuros ou chapéus.</li>
        </ul>
      </div>
    </div>
  </div>
  <div class="fr-photo-sheet-footer">
    <button class="fr-photo-edit-btn" onclick="frPhotoChangeImage()">Editar foto</button>
    <button class="fr-photo-use-btn" id="frPhotoUseBtn" onclick="frPhotoConfirm()">
      <span id="frPhotoUseBtnLabel">Usar essa foto</span>
      <div class="fr-photo-dots" id="frPhotoUseDots" style="display:none;">
        <div class="fr-photo-dot"></div>
        <div class="fr-photo-dot"></div>
        <div class="fr-photo-dot"></div>
      </div>
    </button>
...
function frCheckProhibited(val) {
  ...
  const found = FR_PROHIBITED.some(w => v.includes(w));
  if(found) {
    try { console.warn('[FR_SECURITY] Tentativa de palavra proibida detectada.'); } catch(e){}
  }
  return found;
}
...
function frUpdateProgress() {
  ...
  const bar = document.getElementById('frProgress');
  bar.style.width = pct+'%';
  bar.setAttribute('aria-valuenow', frState.step);
...
function frInitCityAutocomplete() {
  ...
  inp.addEventListener('input', () => {
    ...
    debounceTimer = setTimeout(() => {
      ...
      const matches = FR_CITIES_BR.filter(c => norm(c.n).startsWith(q)).slice(0,8);
  ...
  inp.addEventListener('input', () => { frSaveState(); });
}
...
// Profile photo
let frProfilePhotoDataUrl = null;
...
function frSetupProfilePhoto() {
  ...
  inp.addEventListener('change', e => {
    ...
    reader.onload = ev => {
      frProfilePhotoDataUrl = ev.target.result;
      frOpenPhotoSheet(frProfilePhotoDataUrl);
    };
    reader.readAsDataURL(f);
  });
}

function frOpenPhotoSheet(dataUrl) {
  const sheet = document.getElementById('frPhotoSheet');
  const img = document.getElementById('frPhotoSheetImg');
  img.src = dataUrl;
  sheet.classList.add('show');
  // Reset arc canvas
  setTimeout(() => frDrawArc(0), 50);
}

function frTogglePhotoTips() {
  const modal = document.getElementById('frPhotoTipsModal');
  modal.classList.toggle('show');
}

function frPhotoChangeImage() {
  document.getElementById('frPhotoSheet').classList.remove('show');
...
let frArcAnimFrame = null;
function frDrawArc(progress) {
  const canvas = document.getElementById('frPhotoArcCanvas');
  if(!canvas) return;
  ...
  gradient.addColorStop(0,'#AC865C');
  gradient.addColorStop(0.5,'#E8538A');
  gradient.addColorStop(1,'#AC865C');
  ctx.beginPath();
...
function frPhotoConfirm() {
  const btn = document.getElementById('frPhotoUseBtn');
  const label = document.getElementById('frPhotoUseBtnLabel');
  const dots = document.getElementById('frPhotoUseDots');
  ...
  function animate(now) {
    ...
    frDrawArc(progress);
    if(progress < 1) {
      frArcAnimFrame = requestAnimationFrame(animate);
    } else {
      // Done — close sheet, apply photo
      cancelAnimationFrame(frArcAnimFrame);
      setTimeout(() => {
        const sheet = document.getElementById('frPhotoSheet');
        sheet.classList.remove('show');
        // Apply to profile circle
        const preview = document.getElementById('frProfilePreview');
        const profileLabel = document.getElementById('frProfileLabel');
        if(preview && frProfilePhotoDataUrl) {
          preview.src = frProfilePhotoDataUrl;
          profileLabel.classList.add('has-file');
        }
        // Reset button
        label.style.display = '';
...
function frSetupDocUpload() {
  ...
  frontInp.addEventListener('change', e => {
    ...
    if(frState.docFrontFile && frState.docBackFile) frDisableDocInputs();
  });
  ...
  backInp.addEventListener('change', e => {
    ...
    frSaveState();
    frCheckStep4Complete();
    // Disable after both uploaded
    if(frState.docFrontFile && frState.docBackFile) frDisableDocInputs();
  });
}

function frDisableDocInputs() {
  const front = document.getElementById('frDocFront');
  const back = document.getElementById('frDocBack');
  if(front) front.disabled = true;
...
function frRenderCityPopular() {
  ...
  results.innerHTML = `
    ...
    <div style="padding:16px 0 8px;">
      <button class="fr-custom-city-btn" onclick="frCloseCityOverlay();frShowCustomCity();" style="width:100%;">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
        Minha cidade não está na lista
      </button>
    </div>
...
function frFilterHotelCities(q) {
  ...
  const norm = s => s.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  const lower = norm(q);
  const matches = FR_HOTEL_CITIES.filter(c => norm(c).includes(lower));
  if(!matches.length) {
    results.innerHTML = `
      <div class="fr-city-no-results">
        <div style="font-size:32px;margin-bottom:12px;">🏙️</div>
        <div style="font-weight:600;margin-bottom:6px;">Nenhuma cidade encontrada</div>
        <div style="font-size:0.82rem;margin-bottom:16px;">Tente outra busca.</div>
      </div>
...
function frSelectHotel(idx) {
  ...
  frUpdateHotelBottomSheet();
}

function frUpdateHotelBottomSheet() {
  const sheet = document.getElementById('frHotelBottomSheet');
  if(frState.hotel) {
    const nameEl = document.getElementById('frHotelBsName');
    const dateEl = document.getElementById('frHotelBsDate');
    nameEl.textContent = frState.hotel.name;
    if(frState.checkIn && frState.checkOut) {
      dateEl.textContent = frFormatDate(frState.checkIn) + ' → ' + frFormatDate(frState.checkOut);
    } else {
      dateEl.textContent = 'Selecione as datas acima';
    }
    sheet.classList.add('show');
  } else {
    sheet.classList.remove('show');
...
function frRenderCalendar() {
  ...
  for(let m = 0; m < 3; m++) {
    ...
    for(let d=1;d<=daysInMonth;d++) {
      ...
      const isBeyondMax = date > maxFuture;
      ...
      // Lock days more than 2 days after checkin (only during checkout mode)
      let isLocked = false;
      if(ci && frState.calMode === 'checkout') {
        const maxCheckout = new Date(ci);
        maxCheckout.setDate(maxCheckout.getDate() + 2);
        if(date > maxCheckout || date <= ci) isLocked = true;
      }
      let cls = 'fr-cal-day';
      if(isPast || isBeyondMax) cls += ' disabled';
      else if(isLocked) cls += ' locked';
      if(isCheckIn) cls += ' check-in';
      if(isCheckOut) cls += ' check-out';
      if(inRange) cls += ' in-range';
      const clickable = !isPast && !isBeyondMax;
      html += `<div class="${cls}" ${clickable?`onclick="frSelectCalDay('${dateStr}','${isLocked}')"`:''}>${d}</div>`;
    }
    html += '</div>';
  }
  body.innerHTML = html;
...
function frSelectCalDay(dateStr, isLocked) {
  if(isLocked === 'true') {
  ...
  if(frState.calMode === 'checkin') {
    ...
    const ci = new Date(dateStr + 'T12:00:00');
    const co = new Date(ci);
    co.setDate(co.getDate() + 1);
    const y = co.getFullYear(), mo = String(co.getMonth()+1).padStart(2,'0'), d = String(co.getDate()).padStart(2,'0');
    frState.checkOut = `${y}-${mo}-${d}`;
    ...
    // Enforce max 2 days
    const ci = new Date(frState.checkIn + 'T12:00:00');
    const co = new Date(dateStr + 'T12:00:00');
...
function frCheckStep5Complete() {
  // Bottom sheet is the action trigger now; just update it
  frUpdateHotelBottomSheet();
}

function frStep5Next() {
  if(!frState.hotel || !frState.checkIn || !frState.checkOut) {
    frShowToast('Selecione a cidade, as datas e um hotel.', 'error');
    return;
  }
  document.getElementById('frHotelBottomSheet').classList.remove('show');
  frShowLoading(['Confirmando seleção...','Verificando disponibilidade...','Quase lá!'], () => {
    frShowStep(6);
  }, 600);
}
...
function frPopulateCheckout() {
  ...
  // Name — use full name
  const fullName = frState.userData.nome || 'Hóspede';
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              frState.completed = true;
              frState.userData = {};
              frSaveState();

              // Show success screen after 2.2s
              setTimeout(() => {
                ...
                const ssHotel = document.getElementById('frKycSsHotel');
                const ssHotelName = document.getElementById('frKycSsHotelName');
                if(ssHotel) ssHotel.textContent = hotelName;
                if(ssHotelName) ssHotelName.textContent = hotelName;
                frShowStep(9);
              }, 2200);
            }, 2500);
          }, 2500);
...
// ===== SUPPORT BUTTON =====
function frOpenSupport() {
  frShowWarning('Falar com suporte', 'Para falar com nosso suporte, entre em contato pelo e-mail suporte@frameag.com ou pelo WhatsApp disponível no site frameag.com.');
}

// ===== RESUME PROGRESS =====
function frResumeProgress() {
  document.getElementById('frResumeOverlay').classList.remove('show');
  frShowStep(frState.step || 1);
}

function frResetProgress() {
  localStorage.removeItem('fr_sched_state');
  frState = {
    step: 1, totalSteps: 8, userData: {}, hotel: null, hotelCity: null,
    checkIn: null, checkOut: null, upsell1: false, upsell2: false,
    docFrontFile: null, docBackFile: null, docType: null,
    warningCount: 0, blockUntil: null, calMode: 'checkin',
    savedAt: null, completed: false, sensitiveDataCleared: false,
  };
  document.getElementById('frResumeOverlay').classList.remove('show');
  frShowStep(1);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if(s) {
      ...
      const now = Date.now();
      const maxAge = p.completed ? 7*24*60*60*1000 : 1*24*60*60*1000;
      if(p.savedAt && (now - p.savedAt) < maxAge && p.step && p.step > 1) {
        setTimeout(() => {
  ...
  // CPF mask
  document.getElementById('frCpf').addEventListener('input', function(e) {
    frCpfMask(e);
    frSaveState();
  });
  ...
  const tooltipMap = {
    'frNome': 'frNomeTooltip',
    'frCpf': 'frCpfTooltip',
    'frDisplayName': 'frDisplayNameTooltip',
  };
  Object.entries(tooltipMap).forEach(([inputId, tooltipId]) => {
    const inp = document.getElementById(inputId);
    const tip = document.getElementById(tooltipId);
    if(!inp || !tip) return;
    inp.addEventListener('focus', () => tip.classList.add('visible'));
    inp.addEventListener('blur', () => tip.classList.remove('visible'));
  });

  // Step 1 complete check + state save on all inputs
  ['frNome','frRua','frBairro','frNumero','frCidade'].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.addEventListener('input', () => { frCheckStep1Complete(); frSaveState(); });
  });
  ...
  // Terms start checked, sync visual state
  const termsInp = document.getElementById('frTerms');
  const termsBox = document.getElementById('frTermsBox');
  termsInp.checked = true;
  termsBox.classList.add('checked');

  termsInp.addEventListener('change', e => {
    if(e.target.checked) termsBox.classList.add('checked');
    else termsBox.classList.remove('checked');
    frCheckStep2Complete();
  });
  termsBox.addEventListener('click', () => {
    termsInp.checked = !termsInp.checked;
    termsInp.dispatchEvent(new Event('change'));
  });

  document.getElementById('frStep2Btn').addEventListener('click', frStep2Next);
  frSetupProfilePhoto();

  // Pre-enable step 2 button if terms already checked
  frCheckStep2Complete();
  ...
  document.getElementById('frCitySearch').addEventListener('input', e => {
    ...
    if(q.length > 0) {
      ...
      citySearchDebounce = setTimeout(() => frFilterHotelCities(q), 200);
    } else {
  ...
  document.addEventListener('keydown', e => {
    if(e.key==='Escape') {
      ...
      frCloseCalendar();
      frCloseWarning();
      document.getElementById('frPhotoTipsModal').classList.remove('show');
    }
  });
  ...
  // Show step 1 (or resume will handle redirect)
  frShowStep(1);
});