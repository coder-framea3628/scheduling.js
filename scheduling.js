...
.city-option::before {
  content: '';
  background: url('data:image/svg+xml;base64,') no-repeat center;
  width: 16px;
  height: 16px;
}
.hotel-list {
...
.custom-option[data-option="frame"] strong {
  background: var(--primary-color);
  color: #fff;
  padding: 8px 16px;
  border-radius: 24px;
...
.custom-option[data-option="self"] strong {
  border: 1px solid var(--primary-color);
  color: var(--text-color);
  padding: 8px 16px;
  border-radius: 24px;
...
@keyframes slideInOut {
  ...
  10% { opacity: 1; left: calc(50% - 150px); }
  90% { opacity: 1; left: calc(50% - 150px); }
...
#profile-preview-popup .popup-content {
  text-align: center;
}
...
#profile-preview-popup .delete-profile {
  color: var(--error-color);
  cursor: pointer;
  margin-top: 12px;
}
#model-finder-popup .popup-content {
  text-align: center;
}
.model-cards {
  display: grid;
...
const popularCities = [
  ...
  { name: 'Rio de Janeiro, RJ', label: 'Melhores hotéis' },
...
document.body.innerHTML = `
  <div class="container">
    ...
    <div id="step2" class="step">
      ...
      <label class="upload">
        ...
        <div class="circle-upload" id="profileCircle">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2Z"/>
            <path d="M4.271 18.3457C4.271 18.3457 6.50035 15.0002 12 15.0002C17.4997 15.0002 19.729 18.3457 19.729 18.3457"/>
    ...
    <div id="step6" class="step">
      ...
      <div class="resumo-card">
        <div class="info-row"><span>Nome:</span><strong id="resumo-nome"></strong></div>
        <div class="info-row"><span>Datas:</span><strong id="resumo-dates"></strong></div>
        <div class="info-row"><span>Local:</span><strong id="resumo-hotel"></strong></div>
        <div class="info-row"><span>Verificação facial:</span><strong>Pendente</strong></div>
      </div>
      <div class="hospedagem-option">
        <input type="checkbox" id="add-hospedagem-checkbox">
        <label for="add-hospedagem-checkbox">Adicionar hospedagem completa (incluído no valor Frame)</label>
      </div>
      <div class="create-account-checkbox">
        <input type="checkbox" id="create-account">
        <label for="create-account">Criar minha conta e acompanhar o andamento</label>
      </div>
    ...
    <div id="account-section" class="step">
      <div class="account-greeting">
        <h3>Olá, <span id="account-name"></span></h3>
        <small>Contratante</small>
      </div>
      <div class="discover-models">
        <p>Descubra modelos verificadas agora</p>
        <button onclick="window.location.href='https://frameag.com/models'">Surpreenda-se</button>
      </div>
      <div class="account-options">
        <div class="account-option" onclick="showAccountSettings()">Configurações da conta</div>
        <div class="account-option" onclick="showProfilePreview()">Ver perfil</div>
        <div class="account-option" onclick="window.location.href='https://frameag.com/privacy'">Privacidade</div>
        <div class="account-option" onclick="window.location.href='https://frameag.com/chat'">Receber ajuda</div>
        <div class="account-option" onclick="showFavoritedHotels()">Visualizar hotéis favoritados</div>
        <div class="account-option" onclick="showModelFinder()">Encontrar modelo</div>
        <div class="account-option" onclick="window.location.href='https://frameag.com/framechat'">FrameChat</div>
        <div class="account-option" onclick="window.location.href='https://frameag.com/contato'">Contato</div>
        <div class="account-option" onclick="window.location.href='https://frameag.com/contato/juridico'">Jurídico</div>
        <div class="account-option" onclick="logout()">Sair da conta</div>
      </div>
    </div>
  </div>
  <div class="date-fullscreen" id="date-fullscreen">
    <div class="calendar-header">
      <button class="btn secondary" id="close-dates" style="font-size: 0.8rem; padding: 8px 16px;">Fechar</button>
  ...
  <div class="popup-overlay" id="verification-popup">
    <div class="popup-content">
      <p id="verification-instructions">Clique em Iniciar para começarmos a validação da sua identidade.</p>
      <div id="verification-messages">
  ...
  <div id="selected-summary-popup">
    <h2>Data escolhida</h2>
    <p id="selected-dates"></p>
    <p id="selected-hotel"></p>
    <button class="btn" onclick="closeSelectedSummary()">Prosseguir</button>
  </div>
  <div id="upsell-modal">
    <div class="modal-content">
      <h2>Adicionar hospedagem completa</h2>
      <p>Confirme para adicionar ao pagamento.</p>
      <button class="btn" onclick="generatePayment()">Gerar pagamento</button>
      <button class="btn secondary" onclick="closeUpsellModal()">Cancelar</button>
    </div>
  </div>
  <div id="account-creation-overlay">
    <div class="spinner"></div>
    <p id="account-creation-message">Criando sua conta...</p>
  </div>
  <div class="popup-overlay" id="profile-preview-popup">
    <div class="popup-content">
      <h2>Preview do contratante</h2>
      <p>É assim que a criadora vê você</p>
      <img id="preview-photo" src="" alt="Foto de perfil">
      <p id="preview-name"></p>
      <p>Data de abertura: <span id="account-creation-date"></span></p>
      <div class="delete-profile" onclick="deleteProfile()">Excluir perfil</div>
      <span class="close-popup">×</span>
    </div>
  </div>
  <div class="popup-overlay" id="model-finder-popup">
    <div class="popup-content">
      <h2>Sua companhia certa</h2>
      <div class="model-cards">
        <div class="model-card">
          <img src="https://" alt="Modelo 1">
          <div class="overlay">Encontre agora</div>
        </div>
        <div class="model-card">
          <img src="https://" alt="Modelo 2">
          <div class="overlay">Encontre agora</div>
        </div>
        <div class="model-card">
          <img src="https://" alt="Modelo 3">
          <div class="overlay">Encontre agora</div>
        </div>
        <div class="model-card">
          <img src="https://" alt="Modelo 4">
          <div class="overlay">Encontre agora</div>
        </div>
      </div>
      <button class="btn" onclick="window.location.href='https://frameag.com/models'">Visitar catálogo</button>
      <span class="close-popup">×</span>
    </div>
  </div>
...
let favoritedHotels = JSON.parse(localStorage.getItem('favoritedHotels')) || [];
...
function showStep(step) {
  ...
  const firstInput = document.querySelector('.step.active input');
  if (firstInput) firstInput.focus();
  if (step === totalSteps + 1) { // Account section
...
function loadAccountData() {
  const firstName = userData.personal.nome.split(' ')[0];
  document.getElementById('account-name').textContent = firstName;
  document.getElementById('preview-photo').src = userData.profile.profilePic || '';
  document.getElementById('preview-name').textContent = userData.profile.displayName;
  const creationDate = new Date(localStorage.getItem('accountCreationDate')).toLocaleDateString('pt-BR');
  document.getElementById('account-creation-date').textContent = creationDate;
}
function showAccountSettings() {
  // Implement editing logic, masking sensitive data
  showWarningPopup('Configurações da conta (implementar edição com máscara para dados sensíveis).');
}
function showProfilePreview() {
  document.getElementById('profile-preview-popup').classList.add('show');
}
function deleteProfile() {
  if (confirm('Tem certeza que deseja excluir o perfil?')) {
    localStorage.clear();
    showStep(1);
  }
}
function showFavoritedHotels() {
  showWarningPopup('Hotéis favoritados: ' + favoritedHotels.join(', '));
}
function showModelFinder() {
  document.getElementById('model-finder-popup').classList.add('show');
}
function logout() {
...
function checkProhibited(inputValue) {
  ...
  if (prohibitedWordsList.includes(texto)) {
    ...
    if (warningCount >= 4) {
      ...
      showWarningPopup(`Aviso ${warningCount}/3: Palavra inadequada detectada. Por favor, evite termos ofensivos.`);
      return true;
    }
  }
  return false;
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (checkProhibited(nome) || checkProhibited(rua) || checkProhibited(bairro) || checkProhibited(numero) || checkProhibited(cidade) || checkProhibited(email)) {
      valid = false;
    }
    ...
    if (valid) {
      userData.personal = { nome, email, cpf, rua, bairro, numero, cidade, estado };
    }
  } else if (step === 2) {
    ...
    if (!checkInDate || !checkOutDate) {
      showWarningPopup('Selecione datas de check-in e check-out.');
      valid = false;
    }
...
function checkLocalStorageUsage() {
  ...
  if (used / total > 0.8) {
    showWarningPopup('Armazenamento local quase cheio. Limpando dados antigos.');
  ...
  if (file && validTypes.includes(file.type) && file.size <= 5 * 1024 * 1024) {
    ...
    reader.onload = (e) => {
      ...
      if (targetId.includes('doc')) {
        ...
        userData.profile.profilePic = e.target.result;
        localStorage.setItem('userData', JSON.stringify(userData));
      }
    };
    reader.readAsDataURL(file);
  } else {
    showWarningPopup('Por favor, envie uma imagem no formato JPEG ou PNG, max 5MB.');
  }
}
...
function renderCalendar(month, year) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (checkInDate && checkInDate.getTime() === dayDate.getTime()) {
      ...
      day.style.background = 'rgba(171,134,91,0.3)';
...
function selectDate(date) {
  ...
  if (!checkInDate) {
    checkInDate = date;
    showToast('Selecionou o Check-in');
  } else if (!checkOutDate) {
    const daysDiff = (date - checkInDate) / (1000 * 60 * 60 * 24);
    if (daysDiff > 2 || daysDiff < 1) return;
    checkOutDate = date;
    ...
    hotels[city].forEach((hotel, index) => {
      ...
      card.innerHTML = `
        <div class="hotel-img-container">
          <img data-src="${hotel.profileImg}" class="hotel-img" alt="${hotel.name}">
        ...
        <div class="hotel-actions">
          <button class="btn" id="select-hotel-btn-${index}" onclick="handleSelectHotel(this, '${city}', ${index})">Selecionar este hotel</button>
...
function handleSelectHotel(btn, city, index) {
  btn.innerHTML = '<div class="spinner"></div>';
  setTimeout(() => {
    selectHotel(city, index);
    btn.innerHTML = 'Selecionar este hotel';
    document.getElementById('selected-dates').textContent = document.getElementById('dates').value;
    document.getElementById('selected-hotel').textContent = hotels[city][index].name;
    document.getElementById('selected-summary-popup').classList.add('show');
  }, 2000);
}
function closeSelectedSummary() {
  document.getElementById('selected-summary-popup').classList.remove('show');
  showStep(6);
}
function toggleFavorite(el, city, index) {
  ...
  if (el.classList.contains('favorited')) {
    favoritedHotels.push(hotels[city][index].name);
  } else {
    favoritedHotels = favoritedHotels.filter(name => name !== hotels[city][index].name);
  }
  localStorage.setItem('favoritedHotels', JSON.stringify(favoritedHotels));
...
function startVerification() {
  ...
  document.getElementById('verification-messages').style.display = 'flex';
  const messages = ['Verificando rosto…', 'Confirmando identidade…', 'Finalizando verificação…'];
  let index = 0;
  const interval = setInterval(() => {
    document.getElementById('verification-message').textContent = messages[index];
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } })
    .then(s => {
      ...
      setTimeout(() => {
        clearInterval(interval);
        document.getElementById('verification-messages').style.display = 'none';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('add-hospedagem-checkbox').addEventListener('change', (e) => {
    if (e.target.checked) addHospedagem();
  });
  document.getElementById('continue-btn').addEventListener('click', () => {
    const createAccount = document.getElementById('create-account').checked;
    if (createAccount) {
      createAccountFunction();
    } else {
      if (userData.hospedagem) {
        goToPayment();
      } else {
        skipHospedagem();
      }
  ...
  if (accountCreated) {
    showStep(totalSteps + 1); // Show account section
  }
});
function createAccountFunction() {
  document.getElementById('account-creation-overlay').style.display = 'flex';
  const messages = ['Criando sua conta…', 'Deixando tudo perfeito!'];
  let index = 0;
  const interval = setInterval(() => {
    document.getElementById('account-creation-message').textContent = messages[index];
    index = (index + 1) % messages.length;
  }, 1500);
  setTimeout(() => {
    clearInterval(interval);
    document.getElementById('account-creation-overlay').style.display = 'none';
