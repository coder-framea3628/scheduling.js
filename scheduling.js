...
.bottom-sheet.show + .popup-overlay {
  display: block;
...
.hotel-favorite.favorited .heart-icon {
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step2" class="step">
      ...
      <label class="upload">
        ...
        <div class="circle-upload" id="profileCircle">
          <svg class="plus-icon" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
    ...
    <div id="step5" class="step">
      ...
      <div class="explanatory-card">
        <img class="explanatory-img" src="https://framerusercontent.com/images/fkQ25pvFwNUzjrzPp50famYE.png" alt="Comunicado Oficial">
        <div class="explanatory-info">
          <div class="explanatory-title">A sua experiência Frame.</div>
          <p class="explanatory-desc">Clique no botão 'Buscar cidade' e digite a cidade onde deseja a realização do seu encontro ou selecione 'Minha cidade não está na lista' e siga os passos indicados.</p>
        </div>
        <div class="explanatory-actions">
          <div class="faq-link" onclick="showFaq()">Acessar a FAQ <svg width="16" height="16" fill="var(--primary-color)"><path d="M4 9h8l-4-4-4 4"/></svg></div>
        </div>
      </div>
    ...
    <div id="step6" class="step">
      ...
      <div class="tudo-incluso">✓ TUDO INCLUSO</div>
      <div class="hotel-header">
        <img id="hotel-photo" class="hotel-photo" src="" alt="Hotel">
        <div>
          <div class="hotel-name" id="resumo-hotel"></div>
          <div class="hotel-rating">5,0 (+900) <svg class="star-icon" viewBox="0 0 24 24"><path d="m12 2 3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></div>
        </div>
      </div>
      ...
      <div class="upsell-card" id="upsell1-card" onclick="toggleUpsell('upsell1')">
      ...
      <div class="upsell-card" id="upsell2-card" onclick="toggleUpsell('upsell2')">
  ...
  <div class="date-fullscreen" id="date-fullscreen">
    <div class="progress-bar"><div class="progress-fill"></div></div>
    <h2 style="text-align: center; margin: 16px 0;">Selecione a data da experiência</h2>
  ...
  <div class="bottom-sheet" id="hotel-popup">
    ...
    <div class="popup-overlay" onclick="closeBottomSheet('hotel-popup')"></div>
    <div class="hotel-popup-header">
      <img class="hotel-popup-img" src="" alt="">
    </div>
    <div class="hotel-popup-content">
      <h3 class="hotel-popup-title"></h3>
      <p class="hotel-popup-desc"></p>
      <div class="hotel-popup-services"></div>
    </div>
    <button class="btn hotel-popup-btn">Selecionar hotel</button>
  </div>
  <div class="bottom-sheet" id="custom-city-popup">
    <span class="close-popup">×</span>
    <div class="popup-overlay" onclick="closeBottomSheet('custom-city-popup')"></div>
  ...
  <div class="selected-summary" id="selected-summary">
    ...
    <button class="btn" id="proceed-btn" disabled>Prosseguir</button>
  </div>
  <div class="bottom-sheet payment-popup" id="payment-popup">
    <span class="close-popup">×</span>
    <div class="popup-overlay" onclick="closeBottomSheet('payment-popup')"></div>
    <h3>Instruções</h3>
  ...
  <div class="bottom-sheet" id="faq-popup">
    <span class="close-popup">×</span>
    <div class="popup-overlay" onclick="closeBottomSheet('faq-popup')"></div>
    <div class="faq-content">
      <h2>Perguntas Frequentes</h2>
      <h3>Como funciona a experiência?</h3>
      <p>A experiência inclui hospedagem e encontro presencial.</p>
      <h3>Posso cancelar?</h3>
      <p>Consulte a política de reembolso.</p>
      <!-- Add more FAQs -->
    </div>
  </div>
  <div class="loading-overlay" id="success-overlay">
    <div class="spinner"></div>
...
function initAutocomplete() {
  ...
  input.addEventListener('input', () => {
    ...
    filtered.forEach(c => {
      ...
      option.innerHTML = `<svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>${c}`;
    ...
    hotels[city].forEach((hotel, index) => {
      ...
      card.innerHTML = `
        <div class="hotel-img-container">
          ...
          <span class="hotel-favorite ${favorites.includes(city + index) ? 'favorited' : ''}" onclick="toggleFavorite(this, '${city}', ${index})"><svg class="heart-icon" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></span>
        </div>
        ...
        <div class="hotel-actions">
          <button class="btn" onclick="selectHotel('${city}', ${index}, this)"><span class="btn-text">Selecionar hotel</span></button>
...
function showHotelDetails(city, index) {
  ...
  const popup = document.getElementById('hotel-popup');
  popup.querySelector('.hotel-popup-img').src = hotel.largeImg;
  popup.querySelector('.hotel-popup-title').textContent = hotel.name;
  popup.querySelector('.hotel-popup-desc').textContent = hotel.desc;
  const servicesGrid = popup.querySelector('.hotel-popup-services');
  servicesGrid.innerHTML = '';
  hotel.services.forEach(s => {
    const item = document.createElement('div');
    item.className = 'service-item';
    item.innerHTML = `<svg class="service-icon" width="24" height="24" fill="var(--primary-color)"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"/></svg>${s}`;
    servicesGrid.appendChild(item);
  });
  popup.querySelector('.hotel-popup-btn').onclick = () => selectHotel(city, index, popup.querySelector('.hotel-popup-btn'));
...
function updateUpsell() {
  let total = 0;
  if (document.getElementById('upsell1-card').classList.contains('checked')) total += 470;
  if (document.getElementById('upsell2-card').classList.contains('checked')) total += 525;
  if (total > 0) {
...
confirmBtn.addEventListener('click', () => {
  let upsellTotal = 0;
  if (document.getElementById('upsell1-card').classList.contains('checked')) upsellTotal += 470;
  if (document.getElementById('upsell2-card').classList.contains('checked')) upsellTotal += 525;
  if (upsellTotal > 0) {
    showBottomSheet('payment-popup');
  } else {
    showStep(7);
  }
});
document.getElementById('go-to-payment').addEventListener('click', () => {
  ...
  closeBottomSheet('payment-popup');
  confirmBtn.textContent = 'Pagamento efetuado';
  confirmBtn.classList.add('warm');
  confirmBtn.onclick = () => {
    showToast('O pagamento será verificado junto com os dados e ela segue o fluxo para a próxima etapa.');
    showLoadingOverlay(['Verificando pagamento...'], () => showStep(7));
  };
});
...
document.getElementById('step7-btn').addEventListener('click', () => {
  window.location.href = 'https://frameag.com/kyc/demo';
});
function showFaq() {
  showBottomSheet('faq-popup');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('step3-btn').addEventListener('click', () => { if (validateStep(3)) {
    ...
    showSuccessOverlay(['Verificando informações...', 'Analisando seus dados...', 'Tudo certo!'], () => showStep(4));
  } });
  document.getElementById('step4-btn').addEventListener('click', () => { if (validateStep(4)) showSuccessOverlay(['Verificando informações...', 'Analisando seus dados...', 'Tudo certo!'], () => {
    showCalendar();
  }); });
