...
.hotel-fullscreen.show {
  display: flex;
  transform: translateY(0);
}
.hotel-fullscreen-header {
  padding: 20px;
  position: sticky;
  top: 0;
  background: var(--secondary-bg);
...
.hotel-fullscreen-title {
  font-size: 1.2rem;
  font-weight: 600;
}
/* Libera clique e seleção em tudo que for interativo */
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step1" class="step active">
      ...
      <div class="row">
        ...
        <div class="input-group">
          ...
          <input type="text" id="numero" placeholder="Ex: 143" inputmode="numeric" maxlength="100">
          <div class="error-message"></div>
        </div>
      </div>
      <div class="row">
    ...
    <div id="step5" class="step">
      <h2>Selecionar cidade e hospedagem</h2>
      ...
      <div class="comunicado-card">
        ...
        <div class="hotel-actions">
          ...
          <div class="details-link" onclick="document.getElementById('city-input').focus()"></div>
        </div>
      </div>
<div class="city-select" style="display: flex; flex-direction: column; gap: 10px; margin-top: 15px;">
  <label for="city-input" style="font-weight: 600;">Cidade</label>
  ...
  <div class="input-wrapper" style="position: relative; width: 100%;">
    ...
    <input 
      ...
      style="width: 100%; padding: 10px 10px 10px 36px; border-radius: 10px; border: 1px solid #ddd; font-size: 0.9rem; outline: none;"
    >
  </div>
  ...
  <!-- Botão continuar logo abaixo -->
  <button id="continue-btn" class="btn primary" style="width: 100%; padding: 10px 0; border-radius: 10px; font-weight: 500;">Continuar</button>
</div>
...
<div class="calendar-header">
  <div class="calendar-top-title" style="width: 100%; text-align: center; font-weight: 600; margin-bottom: 4px; font-size: 1rem;">Data da experiência</div>
  ...
  <div class="hotel-fullscreen" id="hotel-fullscreen">
    <div class="hotel-fullscreen-header">
      <button class="btn secondary" onclick="closeHotelFullscreen()" style="font-size: 0.8rem; padding: 6px 12px;">Fechar</button>
      <div class="hotel-fullscreen-title" id="hotel-city-title"></div>
    </div>
    <div id="hotel-list-fullscreen" class="hotel-list" style="padding: 20px; overflow-y: auto;"></div>
  </div>
  ...
  <div class="faq-popup" id="faq-popup">
    ...
    <ul class="faq-list">
      <li><div class="faq-question">Como cancelar?</div><div class="faq-answer">Envie um e-mail pra: <a href="mailto:contato@frameag.com">contato@frameag.com</a> com 48h de antecedência solicitando o cancelamento da experiência e do contrato, responderemos você com os próximos passos.</div></li>
    ...
    <button class="btn" onclick="closePopup('faq-popup')">Fechar</button>
  </div>
  <div class="premium-overlay" id="premium-overlay">
    <div class="premium-content">
      ...
      <button class="btn premium-btn" onclick="acceptPremium()">Sim, seguir com a oferta</button>
      <div class="premium-secondary" onclick="declinePremium()">Prefiro não seguir</div>
    </div>
  </div>
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (checkProhibited(displayName)) {
      showError('displayName', 'Não é permitido usar palavras inadequadas.');
      valid = false;
    }
    if (!displayName) {
...
function loadHotels(city) {
  const listContainer = document.getElementById('hotel-list-fullscreen');
  listContainer.innerHTML = '';
  document.getElementById('hotel-city-title').textContent = `Hotéis parceiros em: ${city}`;
  if (hotels[city]) {
  ...
  document.getElementById('hotel-fullscreen').classList.add('show');
  document.getElementById('step5-btn').disabled = true;
  document.getElementById('proceed-btn').disabled = !selectedHotel;
}
function closeHotelFullscreen() {
  document.getElementById('hotel-fullscreen').classList.remove('show');
}
...
function showHotelDetails(city, index) {
  ...
  content.innerHTML = `
    ...
    <div>
      ...
      <p style="font-size: 0.9rem; text-align: center;">${hotel.desc}</p>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('overlay-black').addEventListener('click', () => {
    document.querySelectorAll('.bottom-sheet.show, .payment-popup, .faq-popup, .warning-popup, .popup-overlay.show').forEach(el => {
      el.style.opacity = 0;
      setTimeout(() => { el.style.display = 'none'; el.style.opacity = 1; }, 300);
    });
    document.getElementById('overlay-black').classList.remove('show');
  });
  const validatedInputs = document.querySelectorAll('#nome, #rua, #bairro, #numero, #cidade, #displayName');
