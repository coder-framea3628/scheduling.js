...
const hotels = {
  ...
  'Porto Alegre, RS': [
    ...
    },
    {
      ...
      profileImg: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/26/a8/22/22/ae/fachada.jpg',
  ...
  'Curitiba, PR': [
    ...
    },
    {
      ...
      profileImg: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/2a/c3/22/95/lobby.jpg',
  ...
  'Belo Horizonte, MG': [
    ...
    },
    {
      ...
      largeImg: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/2f/62/c0/c2/caption.jpg',
      selo: 'Parceiro oficial da Frame',
      services: ['Wi-Fi', 'Estacionamento', 'Café da manhã', 'Academia', 'Piscina'],
      available: 12
    }
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step1" class="step active">
      ...
      <div class="input-group">
        ...
        <div class="tooltip">Nome completo até 100 caracteres, sem caracteres especiais excessivos.</div>
      ...
      <div class="input-group">
        ...
        <div class="tooltip">Endereço completo para verificação.</div>
        <div class="error-message"></div>
      </div>
      <div class="row">
        <div class="input-group">
          <label for="bairro">Bairro</label>
          <input type="text" id="bairro" placeholder="Ex: Centro" maxlength="50">
          <div class="tooltip">Bairro para localização precisa.</div>
          <div class="error-message"></div>
        </div>
        <div class="input-group">
          <label for="numero">Número</label>
          <input type="text" id="numero" placeholder="Ex: 143" inputmode="numeric" maxlength="10">
          <div class="tooltip">Número da residência.</div>
          <div class="error-message"></div>
        </div>
      </div>
      <div class="row">
        <div class="input-group">
          ...
          <input type="text" id="cidade" placeholder="São Paulo" maxlength="50">
          <div class="tooltip">Cidade de residência.</div>
          <div class="error-message"></div>
        </div>
        <div class="input-group">
    ...
    <div id="step2" class="step">
      ...
      <div class="input-group">
        ...
        <input type="text" id="displayName" placeholder="Ex: João Silva" maxlength="50">
        <div class="tooltip">Nome exibido no perfil, até 50 caracteres.</div>
        <div class="error-message"></div>
      </div>
      <div class="input-group">
    ...
    <div id="step5" class="step">
      ...
      <div class="comunicado-card">
        ...
        <div class="hotel-actions">
          <button class="btn" onclick="showFaq()">Acessar FAQ</button>
        </div>
      </div>
      ...
      <div id="hotel-section" style="display: none;">
        <h3 id="hotel-title" style="font-size: 1rem;">Hotéis parceiros em: </h3>
      </div>
    ...
    <div id="step6" class="step">
      ...
      <div class="pdf-section">
        ...
        <div id="lottie-reservation" style="width: 140px; height: 140px; margin: 0 auto;"></div>
      </div>
    </div>
  ...
  <div class="premium-overlay" id="premium-overlay">
    <div class="premium-content">
      <div id="lottie-premium" style="width: 140px; height: 140px; margin: 0 auto;"></div>
      ...
      <button class="btn premium-btn">Sim, seguir com a oferta</button>
      <div class="premium-no">Seguir sem a oferta</div>
    </div>
  </div>
  <div id="chatbot" onclick="showFaq()">?</div>
`;
// ===== Lógica JavaScript =====
...
function showStep(step) {
  ...
  if (step === 6 && selectedHotel) {
    ...
    document.getElementById('pdf-title').textContent = `${userData.personal.nome}, estamos quase finalizando!`;
    lottie.loadAnimation({
      container: document.getElementById('lottie-reservation'),
      renderer: 'svg',
      loop: true,
      autoplay: true,
      path: 'https://lottie.host/a11872ae-9848-45ba-8871-fa870929372e/YWZlUTBnMU.lottie'
    });
  }
  if (step === 5) {
...
function loadHotels(city) {
  ...
  document.getElementById('hotel-title').textContent = `Hotéis parceiros em: ${city}`;
...
function showHotelDetails(city, index) {
  ...
  content.innerHTML = `
    ...
    <div>
      <h3 style="font-size: 1.2rem; font-weight: bold; text-align: center;">${hotel.name}</h3>
...
confirmBtn.addEventListener('click', () => {
  ...
  document.getElementById('go-payment').onclick = () => {
    const text = document.getElementById('go-payment');
    text.style.display = 'none';
    const spinner = document.createElement('div');
    spinner.className = 'spinner';
    spinner.style.position = 'absolute';
    ...
    spinner.style.top = '50%';
    spinner.style.transform = 'translate(-50%, -50%)';
    text.parentElement.appendChild(spinner);
    window.open(link, '_blank');
    setTimeout(() => {
      text.style.display = 'block';
      text.parentElement.removeChild(spinner);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('step1-btn').addEventListener('click', () => { if (validateStep(1)) {
    ...
    lottie.loadAnimation({
      container: document.getElementById('lottie-premium'),
      renderer: 'svg',
      loop: true,
      autoplay: true,
      path: 'https://lottie.host/8cbce635-93a8-4ae4-b111-14aa9305a5e3/4MraxyYnP2.lottie'
    });
    document.querySelector('.premium-btn').onclick = () => {
      document.getElementById('premium-overlay').style.display = 'none';
      showStep(2);
    };
    document.querySelector('.premium-no').onclick = () => {
      document.getElementById('premium-overlay').style.display = 'none';
      showStep(2);
    };
  } });
...
function generatePDF() {
  ...
  const checkInDia = String(checkInDate.getDate()).padStart(2, '0');
  const checkInMes = String(checkInDate.getMonth() + 1).padStart(2, '0');
  const checkInAno = checkInDate.getFullYear();
  const checkOutDia = String(checkOutDate.getDate()).padStart(2, '0');
  const checkOutMes = String(checkOutDate.getMonth() + 1).padStart(2, '0');
  const checkOutAno = checkOutDate.getFullYear();
  const datas = `${checkInDia}/${checkInMes}/${checkInAno} – ${checkOutDia}/${checkOutMes}/${checkOutAno}`;
  const emissao = `${dia} de ${mes} de ${ano}`;
  const pagoEm = `${dia} de ${mes} de ${ano}`;
  doc.addFont('Montserrat-Regular.ttf', 'Montserrat', 'normal');
  doc.setFont('Montserrat');
  doc.addImage('https://framerusercontent.com/images/2q0L5VgbcB1j70YVtdptjLfg08A.png', 'PNG', 10, 10, 80, 0);
  doc.setFontSize(16);
  doc.text('Pré Reserva - Frame Agency', 105, 20);
  doc.setFontSize(12);
  doc.text(`Número da reserva: ${codigoReserva}`, 10, 40);
  doc.text(`Data de emissão: ${emissao}`, 10, 50);
  doc.text('Frame Tecnologia LTDA', 10, 60);
  doc.text('CNPJ - 38.539.493/0001-05', 10, 70);
  doc.text('Av Brigadeiro Faria Lima', 10, 80);
  doc.text('São Paulo - São Paulo, Brasil', 10, 90);
  doc.text('contato@frameag.com', 10, 100);
  doc.text('Emitida para:', 10, 110);
  doc.text(nome, 10, 120);
  doc.text(cpf, 10, 130);
  doc.text(endereco, 10, 140);
  doc.text('Serviço Hoster de Pré Reserva Frame Tecnologia LTDA', 10, 150);
  doc.text('by Booking.com Brasil Servicos de Reserva de Hoteis LTDA', 10, 160);
  doc.text(`${datas}.`, 10, 170);
  doc.text('Adicione', 10, 180);
  doc.text('1 estádia', 10, 190);
  doc.text('2 hóspedes', 10, 200);
  doc.text('Olá! Essa é uma confirmação da sua Pré Reserva para o serviço de hospedagem da Frame Agency.', 10, 210, { maxWidth: 190 });
  doc.text('Status e Condições:', 10, 230);
  doc.text('Esta reserva é uma Pré-Reserva e está sujeita à verificação interna da Frame Tecnologia LTDA, incluindo a validação', 10, 240, { maxWidth: 190 });
  doc.text('de identidade e documentos. A efetivação final da reserva depende da aprovação integral após esta verificação.', 10, 250, { maxWidth: 190 });
  doc.text('Cancelamento:', 10, 270);
  doc.text('A Frame Tecnologia LTDA se reserva o direito de cancelar unilateralmente esta Pré-Reserva a qualquer momento,', 10, 280, { maxWidth: 190 });
  doc.text('caso as condições e verificações não sejam atendidas, ou por motivos internos da empresa. O cancelamento não', 10, 290, { maxWidth: 190 });
  doc.text('gerará ônus ou responsabilidade adicional para a Frame, exceto pela devolução de valores, se aplicável.', 10, 300, { maxWidth: 190 });
  doc.text('Termos e Condições:', 10, 320);
  doc.text('Ao baixar e/ou possuir este documento, o Usuário citado neste PDF declara ter lido e concordado integralmente com', 10, 330, { maxWidth: 190 });
  doc.text('os Termos e Condições Gerais disponíveis em[](https://frameag.com/termos), bem como os Termos do contrato', 10, 340, { maxWidth: 190 });
  doc.text('assinado digitalmente antes da emissão deste arquivo.', 10, 350, { maxWidth: 190 });
  doc.text('Oferecemos um suporte 24h todos os dias. Fale conosco em: www.frameag.com/contato.', 10, 360, { maxWidth: 190 });
  doc.text('Ou no chat de atendimento no Telegram: https://t.me/suporteframebot?start=fatura-mail', 10, 370, { maxWidth: 190 });
  doc.text('Atenciosamente,', 10, 380);
  doc.text('Frame Tecnologia LTDA.', 10, 390);
  doc.setTextColor('#9A7A56');
  doc.setDrawColor('#9A7A56');
  doc.textWithLink('https://frameag.com/login', 10, 200, { url: 'https://frameag.com/login' });
  doc.line(10, 201, doc.getTextWidth('https://frameag.com/login') + 10, 201);
  doc.textWithLink('www.frameag.com/contato', 10, 360, { url: 'https://frameag.com/contato' });
  doc.line(10, 361, doc.getTextWidth('www.frameag.com/contato') + 10, 361);
  doc.textWithLink('https://t.me/suporteframebot?start=fatura-mail', 10, 370, { url: 'https://t.me/suporteframebot?start=fatura-mail' });
  doc.line(10, 371, doc.getTextWidth('https://t.me/suporteframebot?start=fatura-mail') + 10, 371);
  doc.setTextColor(0);
  doc.text('Subtotal', 140, 260);
  doc.text('R$ 3740,00', 170, 260);
  doc.text('Desconto Partner', 140, 270);
  doc.text('R$ 3740,00', 170, 270);
  doc.text('Valor devido pelo cliente', 140, 280);
  doc.text('R$ 0,00', 170, 280);
  doc.text(`R$ 3740,00 pagos pela empresa em ${pagoEm}.`, 10, 290);
  ...
  continueBtn.onclick = () => showStep(7);
  document.querySelector('.pdf-section').appendChild(continueBtn);
}
updateProgress();```