...
// ===== Injetar jsPDF para Geração de PDF =====
...
.hotel-img, .hotel-name, .hotel-desc {
  user-select: none;
  -webkit-user-drag: none;
}
.hotel-img[oncontextmenu], .hotel-name[oncontextmenu], .hotel-desc[oncontextmenu] {
  return false;
}
.hotel-selo {
  position: absolute;
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step5" class="step">
      ...
      <div class="city-select" style="margin-top: 24px;">
      ...
      <div id="hotel-list" class="hotel-list" style="display: none;"></div>
    ...
    <div id="step6" class="step">
      ...
      <dotlottie-player id="lottie-reservation" src="https://lottie.host/a11872ae-9848-45ba-8871-fa870929372e/YWZlUTBnMU.lottie" background="transparent" speed="1" style="width: 140px; height: 140px; margin: 0 auto;" loop autoplay></dotlottie-player>
    </div>
  ...
  <div class="payment-popup" id="payment-popup">
    ...
    <button class="btn secondary" id="payment-done" style="display:none; background: #D3AD83; color: #fff;">Pagamento efetuado</button>
  </div>
  <div class="faq-popup" id="faq-popup">
    <h3>Perguntas Frequentes</h3>
    <ul><li><strong>Como cancelar?</strong> Envie email pra contato@frameag.com em até 48h antes da experiência e prosseguimos com o reembolso de 80% do valor.</li><li><strong>O que inclui?</strong> Hospedagem premium durante a experiência, privacidade total e suporte 24h.</li><li><strong>Adicionais do checkout valem a pena?</strong> Sim, os adicionais acrescentam luxos, brinquedos ou vantagens como check-in early e café VIP.</li><li><strong>Verificação facial?</strong> Garante segurança; leva 30s.</li></ul>
  ...
  <div id="chatbot" onclick="window.open('https://t.me/suporteframebot', '_blank')">?</div>
`;
// ===== Lógica JavaScript =====
...
function showStep(step) {
  ...
  if (step === 6 && selectedHotel) {
    ...
    document.getElementById('pdf-text').textContent = `${userData.personal.nome}, estamos quase finalizando! É importante que você baixe e guarde o arquivo da sua reserva.`;
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (nome.length > 100 || rua.length > 100 || bairro.length > 100 || numero.length > 20 || cidade.length > 100) {
      showError('nome', 'Comprimento excessivo.');
      valid = false;
    }
    ...
    if (displayName.length > 100) {
      showError('displayName', 'Comprimento excessivo.');
      valid = false;
    }
    ...
    reader.onload = (e) => {
      ...
      tempImg.onload = () => {
        ...
        if (targetId.includes('doc')) {
          checkDocsUploaded();
          localStorage.setItem(targetId, compressed);
...
function loadHotels(city) {
  ...
  document.getElementById('hotel-list').style.display = 'flex';
  if (hotels[city]) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('save-dates').addEventListener('click', () => {
    if (checkInDate && checkOutDate) {
      ...
      showLoadingOverlay(['Salvando as informações...', 'Agendando...'], () => {
        showStep(5);
  ...
  document.getElementById('step1-btn').addEventListener('click', () => { 
    if (validateStep(1)) {
      ...
      document.getElementById('decline-offer').onclick = () => {
        document.getElementById('premium-overlay').style.display = 'none';
        showStep(2);
      };
    }
  ...
  document.getElementById('continue-btn').addEventListener('click', () => showStep(7));
  const tooltipFields = {
    'cpf': 'Dados corretos garantem a não reprovação do agendamento',
    'nome': 'Nome completo como no documento',
    'rua': 'Endereço residencial completo',
    'displayName': 'Nome exibido no perfil'
  };
  Object.keys(tooltipFields).forEach(id => {
    const input = document.getElementById(id);
    const tooltip = document.createElement('div');
    tooltip.className = 'tooltip';
    tooltip.textContent = tooltipFields[id];
...
function generatePDF() {
  ...
  img.src = logoUrl;
  img.onload = () => {
    doc.addImage(img, 'PNG', 150, 10, 40, 20); // Approx 80px width
    ...
    const emissionDate = `${today.getDate().toString().padStart(2, '0')} de ${today.toLocaleString('pt-BR', { month: 'long' })} de ${today.getFullYear()}`;
    const checkIn = checkInDate ? `${checkInDate.getDate().toString().padStart(2, '0')}/${(checkInDate.getMonth() + 1).toString().padStart(2, '0')}/${checkInDate.getFullYear()}` : '';
    const checkOut = checkOutDate ? `${checkOutDate.getDate().toString().padStart(2, '0')}/${(checkOutDate.getMonth() + 1).toString().padStart(2, '0')}/${checkOutDate.getFullYear()}` : '';
    const randomCode = Math.floor(100000 + Math.random() * 900000);
    const dateCode = `${today.getDate().toString().padStart(2, '0')}${ (today.getMonth() + 1).toString().padStart(2, '0')}${today.getFullYear()}`;
    const reservaCode = `F-${randomCode}-${dateCode}`;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    doc.text('Pré Reserva - Frame Agency', 10, 10);
    doc.text(`Número da reserva: ${reservaCode}`, 10, 20);
    doc.text(`Data de emissão: ${emissionDate}`, 10, 30);
    ...
    doc.text(`${checkIn} – ${checkOut}.`, 10, 110);
    doc.text(`R$ 3740,00 pagos pela empresa em ${emissionDate}.`, 10, 120);
    doc.text('Emitida para:', 10, 130);
    doc.text(nome, 10, 140);
    doc.text(cpf, 10, 150);
    doc.text(endereco, 10, 160);
    doc.text('Adicione', 10, 170);
    doc.text('1 estádia', 10, 180);
    doc.text('2 hóspedes', 10, 190);
    doc.text('Acessar portal do cliente', 10, 200);
    doc.setTextColor('#9A7A56');
    doc.textWithLink('https://frameag.com/login', 10, 200, { url: 'https://frameag.com/login' });
    doc.setTextColor(0, 0, 0);
    doc.text('Olá! Essa é uma confirmação da sua Pré Reserva para o serviço de hospedagem da Frame Agency.', 10, 210);
    doc.text('Status e Condições:', 10, 220);
    doc.text('Esta reserva é uma Pré-Reserva e está sujeita à verificação interna da Frame Tecnologia LTDA, incluindo a validação', 10, 230);
    doc.text('de identidade e documentos. A efetivação final da reserva depende da aprovação integral após esta verificação.', 10, 240);
    doc.text('Oferecemos um suporte 24h todos os dias. Fale conosco em:', 10, 250);
    doc.setTextColor('#9A7A56');
    doc.textWithLink('www.frameag.com/contato.', 10, 250, { url: 'https://frameag.com/contato' });
    doc.setTextColor(0, 0, 0);
    doc.text('Ou no chat de atendimento no Telegram:', 10, 260);
    doc.setTextColor('#9A7A56');
    doc.textWithLink('https://t.me/suporteframebot?start=fatura-mail', 10, 260, { url: 'https://t.me/suporteframebot?start=fatura-mail' });
    doc.setTextColor(0, 0, 0);
    doc.text('Atenciosamente,', 10, 270);
    doc.text('Frame Tecnologia LTDA.', 10, 280);
    doc.text('Subtotal R$ 3740,00', 140, 130);
    doc.text('Desconto Partner R$ 3740,00', 140, 140);
    doc.text('Valor devido pelo cliente R$ 0,00', 140, 150);
    doc.text('Cancelamento:', 10, 290);
    doc.text('A Frame Tecnologia LTDA se reserva o direito de cancelar unilateralmente esta Pré-Reserva a qualquer momento,', 10, 300, { maxWidth: 180 });
    doc.text('caso as condições e verificações não sejam atendidas, ou por motivos internos da empresa. O cancelamento não', 10, 310, { maxWidth: 180 });
    doc.text('gerará ônus ou responsabilidade adicional para a Frame, exceto pela devolução de valores, se aplicável.', 10, 320, { maxWidth: 180 });
    doc.text('Termos e Condições:', 10, 330);
    doc.text('Ao baixar e/ou possuir este documento, o Usuário citado neste PDF declara ter lido e concordado integralmente com', 10, 340, { maxWidth: 180 });
    doc.text('os Termos e Condições Gerais disponíveis em', 10, 350, { maxWidth: 180 });
    doc.setTextColor('#9A7A56');
    doc.textWithLink([](https://frameag.com/termos)', 10, 350, { url: 'https://frameag.com/termos' });
    doc.setTextColor(0, 0, 0);
    doc.text(', bem como os Termos do contrato', 10, 360, { maxWidth: 180 });
    doc.text('assinado digitalmente antes da emissão deste arquivo.', 10, 370, { maxWidth: 180 });
