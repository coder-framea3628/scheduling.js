...
// ===== CONSTANTS =====
const FR_PROHIBITED = ['cu','pau','ppk','goza','chupa','puta','kids','kid','baby','bebe','cp','porno','estupro','estuprador','buceta','caralho','foder','fode','transar','sexo','bucetinha','roubar','exterminar','cuzinho','chupetinha','viado','baitola','prostituta','pedofilo','bct','nuds','leitada','fetiche','penis','vagina','pepeka','piroca','gozada','pedofilia','violencia','ameaca','drogas','ilegal','merda','porra','otario','idiota','imbecil','retardado','maldito','safado','vadia','piranha','vagabundo','assassino','matar'];
...
let frState = {
  ...
  warningCount: 0,
  blockUntil: null,
  calMode: 'checkin', // 'checkin' | 'checkout'
};

// Load persisted state
try {
  const s = localStorage.getItem('fr_sched_state');
  if(s) { const p = JSON.parse(s); frState = {...frState,...p}; }
} catch(e){}

const frSaveState = () => {
  try { localStorage.setItem('fr_sched_state',JSON.stringify(frState)); } catch(e){}
};
...
<div class="fr-wrap">
  <div class="fr-card" id="frCard">
    <div class="fr-progress-wrap">
      <div class="fr-progress-track"><div class="fr-progress-fill" id="frProgress"></div></div>
    ...
    <div class="fr-step" id="frStep2">
      ...
      <div class="fr-check-row">
        <div class="fr-check-box" id="frTermsBox">
          <input type="checkbox" id="frTerms">
          ${ICONS.check}
        </div>
    ...
    <div class="fr-step" id="frStep5">
      ...
      <button class="fr-btn fr-btn-primary" id="frStep5Btn" disabled style="margin-top:24px;">
        <span class="fr-btn-label">Prosseguir</span>
        <div class="fr-btn-spinner"></div>
      </button>
    </div>
    ...
    <div class="fr-step" id="frStep6">
      ...
      <div class="fr-incluso-badge">${ICONS.check} TUDO INCLUSO</div>
...
<div class="fr-kyc-overlay" id="frKycOverlay">
  <div class="fr-kyc-box">
    ...
    <div class="fr-kyc-status-top" id="frKycStatusTop">Iniciando câmera...</div>
    <div class="fr-kyc-status-bottom" id="frKycStatusBottom">Aguarde um momento</div>
...
<div class="fr-premium-overlay" id="frPremiumOverlay">
  <div class="fr-premium-box">
    ...
    <div class="fr-premium-badge">${ICONS.star} Membro Premium</div>
...
function frCheckProhibited(val) {
  ...
  return FR_PROHIBITED.some(w => v.includes(w));
}
...
function frValidateStep1() {
  ...
  if(!FR_CPF_REGEX.test(cpf)) {
    frShowErr('frCpf','CPF inválido. Formato: 000.000.000-00');
    valid = false;
  } else {
    frClearErr('frCpf');
  }
...
function frInitCityAutocomplete() {
  ...
  inp.addEventListener('input', () => {
    ...
    const matches = FR_CITIES_BR.filter(c => c.n.toLowerCase().startsWith(q)).slice(0,8);
...
function frRenderCalendar() {
  ...
  for(let m = 0; m < 3; m++) {
    ...
    for(let d=1;d<=daysInMonth;d++) {
      ...
      html += `<div class="${cls}" ${!isPast?`onclick="frSelectCalDay('${dateStr}')"`:''}>${d}</div>`;
    }
    html += '</div>';
  }
  body.innerHTML = html;
...
function frSelectCalDay(dateStr) {
  if(frState.calMode === 'checkin') {
    ...
    frState.checkOut = null; // reset checkout
...
function frCheckStep5Complete() {
  ...
  document.getElementById('frStep5Btn').disabled = !ok;
}
...
function frStep5Next() {
  ...
  frSetBtnLoading('frStep5Btn', true);
  setTimeout(() => {
    frSetBtnLoading('frStep5Btn', false);
    frShowStep(6);
  }, 600);
}
...
function frPopulateCheckout() {
  ...
  // Name
  const firstName = frState.userData.nome ? frState.userData.nome.split(' ')[0] : 'Hóspede';
  document.getElementById('frResumoNome').textContent = firstName;
  // Reset upsells
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['frNome','frRua','frBairro','frNumero','frCidade'].forEach(id => {
    document.getElementById(id).addEventListener('input', frCheckStep1Complete);
  });
  ...
  // Restore step from state (if user refreshed mid-flow, go back to step 1 for security)
  // Always start from step 1 for fresh experience
  frShowStep(1);
});