...
.hotel-selo {
  ...
  top: 12px;
  left: 12px;
  background: rgba(244,232,219,0.9);
  padding: 4px 8px;
  border-radius: 12px;
  font-size: 0.8rem;
  color: var(--text-color);
...
document.body.innerHTML = `
  <div class="container" style="opacity: 0; animation: fadeIn 0.8s ease forwards;">
    ...
    <div id="step7" class="step">
      <lottie-player src="https://lottie.host/a11872ae-9848-45ba-8871-fa870929372e/YWZlUTBnMU.lottie" background="transparent" speed="1" style="width: 140px; height: 140px; margin: 0 auto;" autoplay></lottie-player>
  ...
  <div id="premium-overlay">
    <div class="premium-container">
      ...
      <button class="btn primary" onclick="proceedWithOffer()">Sim, seguir com a oferta</button>
      <a class="secondary-link" onclick="proceedWithoutOffer()">Seguir sem a oferta</a>
    </div>
  </div>
`;
// ===== Lógica JavaScript =====
...
Date(localStorage.getItem('checkOutDate')) : null;
...
function validateStep(step) {
  ...
  if (step === 1) {
    ...
    if (!/^[a-zA-ZÀ-ÿ\s]+$/.test(nome)) {
      showError('nome', 'Apenas letras e espaços permitidos.');
      valid = false;
    }
    if (nome.length > 100) {
      showError('nome', 'Máximo de 100 caracteres.');
      valid = false;
    }
    ...
    if (!/^[a-zA-Z0-9À-ÿ\s\-]+$/.test(rua)) {
      showError('rua', 'Caracteres inválidos.');
      valid = false;
    }
    if (!rua) {
    ...
    if (!/^[a-zA-Z0-9À-ÿ\s\-]+$/.test(bairro)) {
      showError('bairro', 'Caracteres inválidos.');
      valid = false;
    }
    if (!bairro) {
    ...
    if (!/^[a-zA-Z0-9À-ÿ\s\-]+$/.test(cidade)) {
      showError('cidade', 'Caracteres inválidos.');
      valid = false;
    }
    if (!cidade) {
    ...
    if (!/^[a-zA-ZÀ-ÿ\s]+$/.test(displayName)) {
      showError('displayName', 'Apenas letras e espaços permitidos.');
      valid = false;
    }
    if (displayName.length > 100) {
      showError('displayName', 'Máximo de 100 caracteres.');
      valid = false;
    }
    if (!displayName) {
  ...
  if (file && validTypes.includes(file.type) && file.size <= 5 * 1024 * 1024) {
    ...
    reader.onload = (e) => {
      ...
      tempImg.onload = () => {
        ...
        } else if (targetId === 'docFront') {
          userData.docFront = compressed;
        } else if (targetId === 'docBack') {
          userData.docBack = compressed;
        }
        localStorage.setItem('userData', JSON.stringify(userData));
...
function loadHotels(city) {
  ...
  h3.textContent = `Hotéis parceiros em: ${city}`;
  document.getElementById('step5').insertBefore(h3, listContainer);
  if (hotels[city]) {
    hotels[city].forEach((hotel, index) => {
      ...
      card.innerHTML = `
        <div class="hotel-img-container">
          <img src="${hotel.profileImg}" class="hotel-img" alt="${hotel.name}" data-src="${hotel.profileImg}" oncontextmenu="return false;">
...
function proceedWithOffer() {
  ...
  userData.premiumOffer = true;
  localStorage.setItem('userData', JSON.stringify(userData));
  showStep(2);
}
function proceedWithoutOffer() {
  document.getElementById('premium-overlay').style.display = 'none';
  userData.premiumOffer = false;
  localStorage.setItem('userData', JSON.stringify(userData));
  showStep(2);
}
// Event Listeners
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('overlay-black').addEventListener('click', () => {
    document.querySelectorAll('.bottom-sheet.show, .payment-popup, .faq-popup, .warning-popup').forEach(el => el.style.display = 'none');
    document.getElementById('overlay-black').style.display = 'none';
  });
  const tooltipInputs = document.querySelectorAll('input[data-tooltip]');
  ...
  document.getElementById('cpf').dataset.tooltip = 'Dados corretos garantem a não reprovação do agendamento';
  document.getElementById('nome').dataset.tooltip = 'Informe seu nome como no documento';
  document.getElementById('rua').dataset.tooltip = 'Informe o endereço completo';
  document.getElementById('bairro').dataset.tooltip = 'Informe o bairro corretamente';
  document.getElementById('cidade').dataset.tooltip = 'Informe a cidade corretamente';
  document.getElementById('displayName').dataset.tooltip = 'Escolha um nome de exibição apropriado';
});
