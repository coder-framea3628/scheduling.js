...
.day:not(.selected):not(.disabled).dimmed {
  opacity: 0.5;
}
.calendar-actions {
  display: flex;
...
#hotel-popup .services-list {
  text-align: left;
}
#hotel-popup .hotel-details-card {
  max-width: 90%;
  overflow: auto;
}
#step2 .upload-text {
...
document.body.innerHTML = `
  ...
  <div class="bottom-sheet" id="hotel-popup">
    ...
    <div class="popup-content">
      <img src="" class="popup-img" alt="">
      <h3></h3>
      <p></p>
      <div class="services-list"></div>
      <button class="btn" id="select-from-details">Selecionar hotel</button>
    </div>
  </div>
  ...
  <div class="faq-popup" id="faq-popup">
    ...
    <ul class="faq-list">
      <li><div class="faq-question">Como escolher o hotel ideal?</div><div class="faq-answer">Explore as opções disponíveis ou utilize a busca personalizada para encontrar o hotel que melhor atende às suas preferências de localização e serviços.</div></li>
      <li><div class="faq-question">Meus dados estão protegidos?</div><div class="faq-answer">Sim, todos os dados são tratados com o mais alto nível de segurança, conforme nossa Política de Privacidade, utilizando criptografia e protocolos avançados.</div></li>
      <li><div class="faq-question">O que são upsells e como funcionam?</div><div class="faq-answer">Upsells são adicionais opcionais que elevam sua experiência, como estadias estendidas ou itens especiais. Eles podem ser selecionados durante o checkout para personalização.</div></li>
      <li><div class="faq-question">Política de reembolso da Frame?</div><div class="faq-answer">Oferecemos reembolso integral em até 48 horas antes da data agendada, sujeito a verificação. Consulte nossa Política de Reembolso completa para detalhes específicos.</div></li>
      <li><div class="faq-question">Como funciona o agendamento de horários?</div><div class="faq-answer">Após selecionar a data, o horário exato será alinhado via e-mail para garantir a melhor disponibilidade e compatibilidade com sua agenda.</div></li>
    </ul>
...
function showStep(step) {
  ...
  if (step === 5) {
    ...
    if (checkInDate && checkOutDate) {
      document.getElementById('selected-dates').textContent = `Data: ${checkInDate.toLocaleDateString('pt-BR')} - ${checkOutDate.toLocaleDateString('pt-BR')}`;
...
function initAutocomplete() {
  ...
  input.addEventListener('input', () => {
    ...
    filtered.forEach(c => {
      ...
      option.innerHTML = `<svg width="32" height="32" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg><div style="background: var(--note-bg); border-radius: 8px; padding: 8px;">${c}<small>12 hotéis disponíveis</small><span class="badge">${c.includes('SP') ? 'SP' : (c.includes('Rio') ? 'POPULAR' : '')}</span></div>`;
...
function loadHotels(city) {
  document.getElementById('step5').querySelector('h2').textContent = `Hotéis parceiros em: ${city}`;
...
function selectHotel(city, index, btn) {
  if (!btn) btn = document.getElementById('select-from-details');
